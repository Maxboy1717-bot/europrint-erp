...
| Magic-Numbers | Finding #10: Role-name case drift, dead @Roles (~60 files) | BLOCKED-OWNER-DECISION | n/a | 0% (slice 1 done, see M8 note above: `a96b1980` removed 39 confirmed-dead literals) | Confirmed NOT a live security issue (`roles.guard.ts` already case-insensitive + fail-closed since `21d775de`, predates this finding) — code-hygiene debt only, not an authorization vulnerability. 2026-07-07 investigation (this session) confirmed the root cause: **two genuinely-competing `Role` enums share the exact same export name** — `apps/api/src/common/constants/roles.constants.ts`'s `Role` (16 members, LOWERCASE values e.g. `'super_admin'`, imported by 19 files) vs. `apps/api/src/modules/auth/enums/role.enum.ts`'s `Role` (16 DIFFERENT members, UPPERCASE values e.g. `'SUPER_ADMIN'`, includes roles like ADMIN/CEO/CFO/HR_HEAD/PROD_HEAD/WAREHOUSE/ACCOUNTANT absent from the other, imported by only 3 files) — plus ~10 duplicate local `enum Role {}` re-declarations inline in individual controllers and ~759 raw uppercase string literals in `@Roles(...)` decorators that import neither. Live `users.role` data (super_admin×3/director×1/manager×27/employee×1, no CHECK constraint) matches ONLY the lowercase `roles.constants.ts` enum, suggesting it is the de-facto canonical one today — but retiring the uppercase enum + rewriting ~70 files' `@Roles(...)` decorators + collapsing ~10 duplicate local enums is a large, invasive, purely-cosmetic mechanical refactor with real regression risk (each `@Roles(...)` call site must be re-verified against the guard's case-insensitive matching) for zero functional benefit (guard already works correctly either way). **Owner question:** should `roles.constants.ts`'s lowercase `Role` become the SOLE canonical export (retire `auth/enums/role.enum.ts` + all local duplicates + rewrite all literal `@Roles('UPPERCASE', ...)` call sites to import the canonical enum), and is a large mechanical multi-file refactor worth doing now for a change the audit itself confirms has no live security/correctness impact — or should this be deferred indefinitely as accepted tech debt? |
...
| Two-Worlds | Role-catalog finalization (~36 catalogs) | BLOCKED-OWNER-DECISION | n/a | 20% (slice 1 `a96b1980`) | Same root cause and same owner question as Magic-Numbers Finding #10/M8 above (two competing same-named `Role` enums, lowercase vs UPPERCASE) — see that row for the full investigation and the precise question. Not a separate decision. |
