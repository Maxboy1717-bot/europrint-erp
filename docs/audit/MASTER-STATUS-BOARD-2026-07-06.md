...
| **Critical-Correctness** | `docs/audit/CRITICAL-CORRECTNESS-AUDIT-2026-07-06.md` — C1-C4 (payment race/no-tx, IoT tablet TTL, GL UTC date + varchar period-lock, invoice-number Date.now() collision) + HIGH/MEDIUM/LOW batches | C1 DONE + committed; C2-C4 PLAN done, **EXECUTE in progress, C2 next** | `a7f0129e` (C1) | 2026-07-06 | C1 (`record-payment.handler.ts`, `drizzle-finance-invoice.repo.ts`, `drizzle-finance-ops.repo.ts`, `drizzle-finance.repo.ts`, `i-finance.repo.ts`, `finance.dto.ts`, `finance-payments.controller.ts` + new migration `c1-finance-payments-idempotency-2026-07-06.sql`): absolute-SET `updateInvoicePaidAmount` replaced with an atomic guarded UPDATE (`paid_amount+=X WHERE paid_amount+X<=total_amount`) wrapped in one `db.transaction()` with the payment INSERT; added `finance_payments.idempotency_key` (nullable, partial UNIQUE index, pos_movements pattern). GL posting still can't join the tx (GlPostingService opens its own internal one — same constraint as F3/payroll) so it's called AFTER commit, with an explicit `reverseInvoicePayment` compensation on GL failure. Tests 8/8 PASS incl. 2 new (concurrent-overpay race closed, idempotent replay); full finance suite 43/47 passed, 4 pre-existing unrelated failures (payroll.service.spec.ts DI, cashier-payroll.service.spec.ts missing method — neither file touched by C1). C2 `iot-tablet.service.ts`+FE tablet fetch (iot, untouched by F4 so far) is next, C3 `gl-posting.service.ts:188` one-line UTC fix + `drizzle-gl-posting.repo.ts` (finance, confirmed the varchar period-lock compare is ALREADY correct — only the UTC date calc is the real bug, audit's framing partially wrong), C4 `finance-invoices.controller.ts:80,108` + dormant `invoice_number_seq` reuse (finance). Will re-check `git status` for each specific file immediately before editing it, not just once at the start. |
...
| Critical-Correctness | `apps/api/src/modules/iot/**` + FE tablet auth/fetch files (C2: TTL mismatch + refresh endpoint) | claiming now — checking git status before first edit | in progress |
...
| Magic-Numbers | DOC-v1 overall | QUEUED-NOT-STARTED | n/a | 0% | All 30 findings #1-30 pending owner prioritization |
...
| Magic-Numbers | Finding #4: Quarantine routing literal warehouse codes | QUEUED-NOT-STARTED | n/a | 0% | Quarantine safety flow if renamed |
| Magic-Numbers | Finding #5: QC lot auto-fail defect ratio 0.05 | QUEUED-NOT-STARTED | n/a | 0% | Quality accept/reject gate configurability |
...
| Magic-Numbers | Finding #10: Role-name case drift, dead @Roles (~60 files) | QUEUED-NOT-STARTED | n/a | 0% | Authorization checks across ~60 controllers |
...
| i18n | C9 — API error/validation messages | IN-PROGRESS | n/a | UNKNOWN | n/a |
...
| i18n | Top-10 gap #6 (Zod + ~470 backend throws not localized) | QUEUED-NOT-STARTED | n/a | 0% | n/a |
...
| Two-Worlds | Root-cause cluster 3 — KARTA-centric org unification | IN-PROGRESS | d89c87de | 40% | A7 |
...
| Critical-Correctness | 1.1 Payment double-spend (no idempotency/tx) | UNKNOWN | n/a | 0% | Payment double-spend; #1 top-10 |
| Critical-Correctness | 1.2 Duplicate customer payments (SD ledger) | UNKNOWN | n/a | 0% | n/a |
| Critical-Correctness | 1.3 Duplicate invoice numbers (Date.now()) | UNKNOWN | n/a | 0% | Duplicate invoice numbers; #6 top-10 |
| Critical-Correctness | 1.4 WMS oversell (no atomic quantity guard) | UNKNOWN | n/a | 0% | WMS oversell; #3 top-10 |
| Critical-Correctness | 1.5 POS oversell / negative stock (TOCTOU) | UNKNOWN | n/a | 0% | POS oversell; #4 top-10 |
...
| Critical-Correctness | 2.1 GL entryDate UTC day instead of Tashkent | UNKNOWN | n/a | 0% | GL entry mis-dating; #5 top-10 |
| Critical-Correctness | 2.2 GL period lock raw text compare, no cast normalize | UNKNOWN | 9919dc92 | 0% | GL posting into closed period; #5 top-10 |
...
| Critical-Correctness | 6.1 hard DELETE orphaned BOM lines | UNKNOWN | n/a | 0% | Orphaned BOM lines; #9 top-10 |
| Critical-Correctness | 6.2 hard DELETE orphaned OKR key results | UNKNOWN | n/a | 0% | Orphaned OKR key results; #9 top-10 |
...
| Critical-Correctness | 8.1 auto-barcode Math.random suffix no unique constraint | UNKNOWN | n/a | 0% | Duplicate barcodes; #7 top-10 |
| Critical-Correctness | 8.2 pos-stock-issuable COUNT(*)+1 TOCTOU barcode | UNKNOWN | n/a | 0% | Duplicate barcodes; #7 top-10 |
| Critical-Correctness | 8.3 procurement-request PR number COUNT(*)+1 no lock | UNKNOWN | n/a | 0% | Duplicate PR numbers; #10 top-10 |
