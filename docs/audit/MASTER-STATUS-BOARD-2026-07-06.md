...
| **Critical-Correctness** | `docs/audit/CRITICAL-CORRECTNESS-AUDIT-2026-07-06.md` — C1-C4 (payment race/no-tx, IoT tablet TTL, GL UTC date + varchar period-lock, invoice-number Date.now() collision) + HIGH/MEDIUM/LOW batches | C1+C2 DONE + committed; C3-C4 PLAN done, **PAUSED** — owner redirected this session to the Org-Card Remediation loop below; will resume C3/C4 after that loop reaches a stopping point | `a7f0129e` (C1), `f4d17363`+`b29c8bce` (C2) | 2026-07-06 | C1: see prior entry (atomic guarded UPDATE + tx + idempotency key + GL-failure compensation). C2 (`iot-tablet.service.ts`, `iot-tablet.controller.ts`, new `pages/iot/tabletFetch.ts`, + 4 FE call-site files): BE token TTL 8h→12h (matches FE's SESSION_12H_MS instead of shortening it); new `POST /iot/tablet/refresh` (`ignoreExpiration:true` + 24h issue-grace-window + re-checked is_active/card-gate); new shared `tabletFetch` client consolidates 5 previously-duplicated raw-fetch call sites with 401→refresh→retry. 7/7 new unit tests PASS. Also flagged (not part of C2, just observed): the consolidated-status-board's "IoT Kiosk auth-guard BLOCKED-OWNER, 17 routes always 401" entry is STALE — live code already has `TabletTokenGuard` wired to nearly every tablet route; whoever owns that row should correct it. C3/C4 not started yet this pass — plans from the earlier PLAN phase remain valid and unclaimed. |
| **Org-Card Manual-Entry Remediation (G1-G12)** | `docs/audit/ORG-CARD-MANUAL-ENTRY-READINESS-2026-07-06.md` (Parts A-D) + `docs/migration/02-vysotskiy-7-tree.md` (vision) — G1 (HR role missing from write-guard), G2 (employee-assignment split across `employee_org_departments`/`employee_cards`), G3 (node_type vocabulary missing otdel/sektsiya/sektor), G4 (parent-picker + duplicate-card), G5 (legacy mirror-write in sync-helper.ts), G6 (two competing create entry-points), G7 (ЦКП/salary card→payroll wiring), G8 (course↔card binding), G9 (single-tree invariant, DECIDE), G10 (single canonical card table, DECIDE), G11 (mandatory-reason audit), G12 (BE-side confidential-field projection) | STARTING — PLAN phase | — | 2026-07-06 | New loop, just claimed. Will check for overlap with the Two-Worlds loop's root-cause cluster #3 (KARTA-centric org unification) before touching anything, since G5/G9/G10 explicitly reference it. `org-structure.controller.ts` / `org-mutations.repo.ts` / `sync-helper.ts` live under `apps/api/src/modules/**`, which the i18n Tier-2 F4 claim above covers broadly for hardcoded-message edits — will verify F4's workflow hasn't reached these SPECIFIC files (not just check the broad claim) before editing, same discipline as C1/C2. |
