...
p.query("SELECT column_name, data_type, is_nullable FROM information_schema.columns WHERE table_name='ai_interview_sessions' ORDER BY ordinal_position")
  .then(r => { console.log(r.rows.map(x => `${x.column_name}:${x.data_type}(${x.is_nullable})`).join(', ')); p.end(); })
