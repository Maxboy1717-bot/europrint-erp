    ...
    >
      ...
      {entry.worker_type && (() => {
        ...
        return (
          <div className="flex items-center gap-1 flex-wrap">
            ...
            {isMatch === true && <span className="inline-flex items-center gap-0.5 text-[9px] rounded px-1 py-0.5 bg-green-500/10 text-green-400 border border-green-500/30 font-medium">✓ Mos</span>}
            {isMatch === false && <span className="inline-flex items-center gap-0.5 text-[9px] rounded px-1 py-0.5 bg-orange-500/10 text-orange-400 border border-orange-500/30 font-medium">✗ Mos emas</span>}
          </div>
        );
      })()}
      ...
      {expandedCard === entry.id && vac?.portret && (
        <div data-testid={`portret-panel-${entry.id}`} className="border border-purple-500/20 rounded-md p-2.5 bg-purple-500/5 flex flex-col gap-2 text-xs">
          <p className="text-[10px] font-semibold text-purple-400 uppercase tracking-wide">Xodim Portreti</p>
          {vac.portret?.main_purpose && <div><span className="text-[10px] text-muted-foreground">Lavozim maqsadi:</span><p className="text-foreground mt-0.5 leading-snug line-clamp-3">{vac.portret.main_purpose}</p></div>}
          {vac.portret?.problem_solved && <div><span className="text-[10px] text-muted-foreground">Hal qilinadigan muammo:</span><p className="text-foreground mt-0.5 leading-snug line-clamp-2">{vac.portret.problem_solved}</p></div>}
          {vac.portret?.reports_to && <div><span className="text-[10px] text-muted-foreground">Kimga hisobot beradi:</span><p className="text-foreground mt-0.5">{vac.portret.reports_to}</p></div>}
          {(vac.portret?.age_min ?? vac.portret?.age_max) && <div className="flex gap-3"><span className="text-[10px] text-muted-foreground">Yosh:</span><span className="text-foreground text-[10px]">{vac.portret.age_min}–{vac.portret.age_max}</span></div>}
          {vac.portret?.main_duties && <div><span className="text-[10px] text-muted-foreground">Asosiy majburiyatlar:</span><p className="text-foreground mt-0.5 leading-snug line-clamp-3">{vac.portret.main_duties}</p></div>}
          {vac.portret?.expected_result && <div><span className="text-[10px] text-muted-foreground">Kutilgan natija:</span><p className="text-foreground mt-0.5 leading-snug line-clamp-2">{vac.portret.expected_result}</p></div>}
          {(vac.portret?.salary_min != null || vac.portret?.salary_max != null) && (
            <div className="flex gap-3 flex-wrap">
              {vac.portret?.salary_min != null && <div><span className="text-[10px] text-muted-foreground">Min:</span><span className="font-semibold text-green-400 ml-1">{Number(vac.portret.salary_min).toLocaleString("uz-UZ")} so'm</span></div>}
              {vac.portret?.salary_max != null && <div><span className="text-[10px] text-muted-foreground">Max:</span><span className="font-semibold text-green-400 ml-1">{Number(vac.portret.salary_max).toLocaleString("uz-UZ")} so'm</span></div>}
            </div>
          )}
              ...
              <div className="border border-sky-500/30 rounded-lg bg-sky-500/5 overflow-hidden">
                ...
                {isOpen && (
                  <div className="px-2.5 pb-2.5 flex flex-col gap-1.5 text-xs border-t border-sky-500/20">
                    {cp.kompaniya_taqdimoti && <div><span className="text-[10px] text-muted-foreground">Kompaniya:</span><p className="text-foreground leading-snug line-clamp-3 mt-0.5">{cp.kompaniya_taqdimoti}</p></div>}
                    {cp.ish_tartibi && <div><span className="text-[10px] text-muted-foreground">Ish tartibi:</span><p className="text-foreground line-clamp-2 mt-0.5">{cp.ish_tartibi}</p></div>}
                    {cp.instrumentlar && <div><span className="text-[10px] text-muted-foreground">Instrumentlar:</span><p className="text-foreground line-clamp-1 mt-0.5">{cp.instrumentlar}</p></div>}
                    {cp.guruh_javob && <div><span className="text-[10px] text-muted-foreground">Guruh:</span><span className="text-foreground ml-1">{cp.guruh_javob}</span></div>}
                    <div className="flex gap-3 flex-wrap">
                    ...
                    {sinov && <div><span className="text-[10px] text-muted-foreground">Sinov maoshi:</span><span className="text-amber-400 ml-1 text-[10px]">{Number(cp.sinov_maosh_min || 0).toLocaleString("uz-UZ")} – {Number(cp.sinov_maosh_max || 0).toLocaleString("uz-UZ")} so'm</span></div>}
                    {cp.asosiy_maosh && <div><span className="text-[10px] text-muted-foreground">Asosiy maosh:</span><span className="text-green-400 ml-1">{cp.asosiy_maosh}</span></div>}
                    {cp.martaba && <div><span className="text-[10px] text-muted-foreground">Martaba:</span><p className="text-foreground line-clamp-1 mt-0.5">{cp.martaba}</p></div>}
                    <div className="flex gap-3 flex-wrap">
                    ...
                    {cp.sotsial_paket && <div><span className="text-[10px] text-muted-foreground">Sotsial paket:</span><p className="text-foreground line-clamp-1 mt-0.5">{cp.sotsial_paket}</p></div>}
                    {cp.oqutish && <div><span className="text-[10px] text-muted-foreground">O'qitish:</span><p className="text-foreground line-clamp-1 mt-0.5">{cp.oqutish}</p></div>}
                    {cp.jalb_qiluvchi && <div className="border-t border-sky-500/20 pt-1.5 mt-0.5"><span className="text-[10px] text-sky-400 font-medium">Nima uchun biz:</span><p className="text-foreground line-clamp-2 mt-0.5">{cp.jalb_qiluvchi}</p></div>}
                  </div>
                )}
              </div>
            );
          ...
          <button className="text-[10px] text-purple-400 hover:text-purple-300 underline self-start" onClick={() => vac && setPortretVacancy(vac)}>To'liq portretni tahrirlash →</button>
        </div>
      )}
      ...
      {expandedCard === entry.id && !vac?.portret && (
        <div className="border border-border/30 rounded-md p-2.5 text-center text-xs text-muted-foreground">
          ...
          {vac && <button className="text-purple-400 hover:text-purple-300 underline" onClick={() => setPortretVacancy(vac)}>Portret yaratish</button>}
        </div>
      )}
