    ...
    <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-500">
      {!formData.productId ? (
        <div className="text-center py-12 bg-muted/40 rounded-xl border border-dashed border-border">
          ...
          <p className="text-foreground text-lg font-medium">{t.selectProductFirst}</p>
        </div>
      ) : filteredBoms.length === 0 ? (
        <div className="text-center py-12 bg-muted/40 rounded-xl border border-dashed border-border">
          <Package className="h-12 w-12 text-muted-foreground mx-auto mb-4" />
          <p className="text-foreground text-lg font-medium">{t.noBoms}</p>
        </div>
      ) : (
            ...
            >
              <CardHeader className="pb-2">
                <div className="flex justify-between items-start">
                  <CardTitle className="text-[14px] font-semibold text-primary">{item.bom.bomNumber}</CardTitle>
                  <Badge className={item.bom.status === "active" ? "bg-primary text-white" : "bg-muted text-muted-foreground"}>
                    {t[item.bom.status as keyof typeof t] || item.bom.status}
                  </Badge>
                </div>
                <CardDescription className="font-mono text-xs font-bold text-muted-foreground">
