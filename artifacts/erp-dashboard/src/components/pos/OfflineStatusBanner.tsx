    ...
    return (
      <div className="flex items-center gap-2 px-3 py-1.5 bg-orange-50 border-b border-orange-200 text-orange-800 dark:bg-orange-950 dark:border-orange-800 dark:text-orange-200">
        ...
        <Button
          variant="outline"
          size="sm"
          className="ml-2 h-6 text-xs border-orange-300 text-[var(--ep-primary)] hover:bg-orange-100"
          onClick={onSync}
        >
...
export function OfflineHeaderBadge({ isOnline, pendingCount, syncStatus }: OfflineHeaderBadgeProps) {
  if (!isOnline) {
    return (
      <Badge variant="outline" className="flex items-center gap-1 border-amber-400 text-[var(--ep-yellow)] dark:text-amber-400 bg-amber-50 dark:bg-amber-950">
        ...
        Oflayn
        {pendingCount > 0 && (
          <span className="ml-1 bg-[var(--ep-yellow)] text-white text-xs rounded-full px-1.5">{pendingCount}</span>
        )}
      </Badge>
    );
  }
  ...
  if (pendingCount > 0) {
    return (
      <Badge variant="outline" className="flex items-center gap-1 border-orange-400 text-[var(--ep-primary)]">
