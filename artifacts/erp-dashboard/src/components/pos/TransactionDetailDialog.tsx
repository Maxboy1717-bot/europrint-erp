      ...
      <DialogContent data-testid="dialog-view-transaction" className="p-6">
        ...
        <div className="space-y-3 text-sm">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
            <div><span className="text-muted-foreground">{t("tranzaksiya")}</span> <span className="font-mono">{transaction.transactionNumber}</span></div>
            <div><span className="text-muted-foreground">{t("chek")}</span> <span className="font-mono">{transaction.receiptNumber || "-"}</span></div>
            <div><span className="text-muted-foreground">{t("sana1")}</span> {formatDateTime(transaction.createdAt)}</div>
            <div><span className="text-muted-foreground">{t("mijoz")}</span> {transaction.customerName || "-"}</div>
            <div><span className="text-muted-foreground">{t("tolov2")}</span> {paymentMethodLabels[transaction.paymentMethod]?.uz || transaction.paymentMethod}</div>
            <div>
          ...
          <div className="border-t pt-2 space-y-1">
            <div className="flex justify-between"><span>{t("jami")}</span><span>{formatCurrency(transaction.subtotal)}</span></div>
            {transaction.discountAmount > 0 && (
              <div className="flex justify-between"><span>{t("chegirma")}</span><span>-{formatCurrency(transaction.discountAmount)}</span></div>
            )}
          ...
          {transaction.notes && (
            <div className="border-t pt-2"><span className="text-muted-foreground">{t("izoh")}</span> {transaction.notes}</div>
          )}
        </div>
        <DialogFooter>
