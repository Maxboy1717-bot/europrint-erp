        ...
        <div className="space-y-4 py-2">
          ...
          {missingFields.length > 0 && (
            <div className="rounded-md bg-amber-50 dark:bg-amber-950 p-3 text-sm">
              <p className="font-medium text-[var(--ep-yellow)] dark:text-amber-400 mb-1">{t('qcheck.missingFieldsTitle')}</p>
              <ul className="list-disc list-inside text-[var(--ep-yellow)] dark:text-amber-300 space-y-0.5">
          ...
          {preflight && (
            <div className={`rounded-md p-3 text-sm ${preflight.pass ? "bg-green-50 dark:bg-green-950" : "bg-red-50 dark:bg-red-950"}`}>
              <p className={`font-medium mb-1 ${preflight.pass ? "text-[var(--ep-green)] dark:text-green-400" : "text-[var(--ep-red)] dark:text-red-400"}`}>
                {preflight.pass ? t('qcheck.preflightOk') : t('qcheck.preflightFail')}
              </p>
              {preflight.errors.map((e, i) => (
                <p key={i} className="text-[var(--ep-red)] dark:text-red-300">{e}</p>
              ))}
              {preflight.warnings.map((w, i) => (
                <p key={i} className="text-[var(--ep-yellow)] dark:text-amber-300">{w}</p>
              ))}
            </div>
          )}
          ...
          {autoExecuted ? (
            <div className="rounded-md bg-violet-50 dark:bg-violet-950 p-3 space-y-2">
              <div className="flex items-center justify-between text-sm">
                <span className="flex items-center gap-1.5 font-medium text-[var(--ep-purple)] dark:text-violet-300">
                  <Clock className="h-4 w-4" />
                  {t('qcheck.autoCommitTitle')}
                </span>
                <span className="text-[var(--ep-purple)] dark:text-violet-400 font-mono">{remaining}s</span>
              </div>
              <Progress value={countdownPct} className="h-2 bg-violet-200 dark:bg-violet-800 [&>div]:bg-violet-500" />
              <p className="text-xs text-[var(--ep-purple)] dark:text-violet-400">
                {t('qcheck.autoCommitHint', { seconds: remaining })}
              </p>
            </div>
          ) : (
            <div className="rounded-md bg-blue-50 dark:bg-blue-950 p-3 text-sm text-[var(--ep-blue)] dark:text-blue-300">
