...
export function ObligationsTab({ employeeId, cashAdvances = [], loadingCashAdvances }: Props) {
  const { data: loansData, isLoading: loadingLoans } = useQuery({
    ...
    queryFn: async () => {
      try {
        const r = await fetch(`/api/finance/loans?userId=${employeeId}`);
        if (!r.ok) return [];
        return r.json();
      } catch { return []; }
    },
  ...
  return (
    ...
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
        {([
          { label: "Jami avans", value: fmtMoney(totalAdvances), icon: DollarSign, color: "text-blue-500", bg: "bg-blue-50 dark:bg-blue-950/20" },
          { label: "To'langan",  value: fmtMoney(totalRepaid),   icon: CheckCircle, color: "text-green-500", bg: "bg-green-50 dark:bg-green-950/20" },
          {
            label: "Qoldiq qarz",
            value: fmtMoney(outstanding),
            icon: outstanding > 0 ? AlertCircle : CheckCircle,
            color: outstanding > 0 ? "text-red-500" : "text-green-500",
      ...
      <Card>
        ...
        <CardContent>
          {isLoading && <div className="space-y-2">{([1,2,3]).map(i => <Skeleton key={`k-${i}`} className="h-14 w-full" />)}</div>}
          ...
          <div className="space-y-2">
            {(Array.isArray(advances) ? advances : []).map(a => {
              ...
              const statusColor = a.status === "repaid"  ? "bg-green-100 text-green-700" :
                                  a.status === "overdue" ? "bg-red-100 text-red-700"     :
                                                           "bg-blue-100 text-blue-700";
              return (
                <div key={a.id} className="p-4 rounded-lg border border-border/50 bg-surface-container-low space-y-2">
                  <div className="flex items-center justify-between gap-2">
                    <div>
                      <p className="font-medium text-sm text-on-surface">{a.purpose ?? "Avans"}</p>
                      {a.createdAt && (
                    ...
                    <div className="text-right shrink-0">
                      <p className="font-bold text-sm text-on-surface">{fmtMoney(amount)} so'm</p>
          ...
          <CardContent>
            <div className="space-y-2">
              {(Array.isArray(loans) ? loans : []).map((loan) => (
                <div key={loan.id} className="flex items-center justify-between p-3 rounded-lg bg-surface-container-low">
                  <div>
                    <p className="font-medium text-sm">{loan.purpose ?? "Kredit"}</p>
                    <p className="text-xs text-on-surface-variant">{fmtMoney(loan.amount)} so'm</p>
                  </div>
