...
export default function EmployeeProfile() {
  ...
  const { data: passportData, isLoading: loadingPassport } = useQuery<PassportData | null>({
    queryKey: ['/api/hr/employees', id, 'passport'],
    enabled: !!id,
    retry: false,
    queryFn: async () => {
      try {
        return await apiRequest('GET', `/api/hr/employees/${id}/passport`);
      } catch (e) {
  ...
  const { data: bankAccounts, isLoading: loadingBanks } = useQuery<BankAccount[]>({ queryKey: ['/api/hr/employees', id, 'bank-accounts'], enabled: !!id });
  const { data: emergencyContacts, isLoading: loadingEmergency } = useQuery<EmergencyContact[]>({ queryKey: ['/api/hr/employees', id, 'emergency-contacts'], enabled: !!id });
  ...
  const { data: contracts, isLoading: loadingContracts } = useQuery<EmploymentContract[]>({ queryKey: ['/api/hr/employees', id, 'contracts'], queryFn: mkArrFn<EmploymentContract>(`/api/hr/employees/${id}/contracts`), enabled: !!id });
  ...
  const { data: bonuses, isLoading: loadingBonuses } = useQuery<BonusRecord[]>({ queryKey: ['/api/hr/employees', id, 'bonuses'], queryFn: mkArrFn<BonusRecord>(`/api/hr/employees/${id}/bonuses`), enabled: !!id });
  const { data: fines, isLoading: loadingFines } = useQuery<FineRecord[]>({ queryKey: ['/api/hr/employees', id, 'fines'], queryFn: mkArrFn<FineRecord>(`/api/hr/employees/${id}/fines`), enabled: !!id });
  const { data: overtime, isLoading: loadingOvertime } = useQuery<OvertimeRecord[]>({ queryKey: ['/api/hr/employees', id, 'overtime'], queryFn: mkArrFn<OvertimeRecord>(`/api/hr/employees/${id}/overtime`), enabled: !!id });
  const { data: cashAdvances, isLoading: loadingCashAdvances } = useQuery<CashAdvanceRecord[]>({ queryKey: ['/api/hr/employees', id, 'cash-advances'], queryFn: mkArrFn<CashAdvanceRecord>(`/api/hr/employees/${id}/cash-advances`), enabled: !!id });
  const { data: leaveRequests, isLoading: loadingLeaveRequests } = useQuery<LeaveRequest[]>({ queryKey: ['/api/hr/employees', id, 'leave-requests'], queryFn: mkArrFn<LeaveRequest>(`/api/hr/employees/${id}/leave-requests`), enabled: !!id });
  ...
  const { data: businessTrips, isLoading: loadingBusinessTrips } = useQuery<BusinessTrip[]>({ queryKey: ['/api/hr/employees', id, 'business-trips'], queryFn: mkArrFn<BusinessTrip>(`/api/hr/employees/${id}/business-trips`), enabled: !!id });
  ...
  const savePassportMutation = useMutation({ mutationFn: async (d: typeof passportForm) => apiRequest("POST", `/api/hr/employees/${id}/passport`, { ...d, userId: id }), onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['/api/hr/employees', id, 'passport'] }); setPassportDialogOpen(false); toast({ title: tCommon('savedSuccessfully') }); } });
  const saveBankMutation = useMutation({ mutationFn: async (d: typeof bankForm) => apiRequest("POST", `/api/hr/employees/${id}/bank-accounts`, { ...d, userId: id }), onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['/api/hr/employees', id, 'bank-accounts'] }); setBankDialogOpen(false); setBankForm({ bankName: "", accountNumber: "", cardNumber: "", cardHolderName: "", mfo: "", inn: "", isPrimary: true }); toast({ title: tCommon('savedSuccessfully') }); } });
  const saveEmergencyMutation = useMutation({ mutationFn: async (d: typeof emergencyForm) => apiRequest("POST", `/api/hr/employees/${id}/emergency-contacts`, { ...d, userId: id }), onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['/api/hr/employees', id, 'emergency-contacts'] }); setEmergencyDialogOpen(false); setEmergencyForm({ contactName: "", relationship: "", phoneNumber: "", alternativePhone: "", address: "", isPrimary: true }); toast({ title: tCommon('savedSuccessfully') }); } });
  const saveContractMutation = useMutation({ mutationFn: async (d: typeof contractForm) => apiRequest("POST", `/api/hr/employees/${id}/contracts`, { ...d, userId: id, salary: parseFloat(d.salary) || 0 }), onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['/api/hr/employees', id, 'contracts'] }); setContractDialogOpen(false); setContractForm({ contractNumber: "", contractType: "indefinite", startDate: "", endDate: "", salary: "", workSchedule: "" }); toast({ title: tCommon('savedSuccessfully') }); } });
  ...
  const saveBonusMutation = useMutation({ mutationFn: async (d: typeof bonusForm) => apiRequest("POST", `/api/hr/employees/${id}/bonuses`, { ...d, userId: id, amount: parseFloat(d.amount) || 0 }), onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['/api/hr/employees', id, 'bonuses'] }); setBonusDialogOpen(false); setBonusForm({ paymentDate: "", amount: "", bonusType: "performance", description: "" }); toast({ title: tCommon('savedSuccessfully') }); } });
  const saveFineMutation = useMutation({ mutationFn: async (d: typeof fineForm) => apiRequest("POST", `/api/hr/employees/${id}/fines`, { ...d, userId: id, amount: parseFloat(d.amount) || 0 }), onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['/api/hr/employees', id, 'fines'] }); setFineDialogOpen(false); setFineForm({ fineDate: "", amount: "", fineType: "late", description: "", deductedFromSalary: false }); toast({ title: tCommon('savedSuccessfully') }); } });
  const saveOvertimeMutation = useMutation({ mutationFn: async (d: typeof overtimeForm) => { const h = parseFloat(d.hours) || 0; const r = parseFloat(d.hourlyRate) || 0; const m = parseFloat(d.multiplier) || 1; return apiRequest("POST", `/api/hr/employees/${id}/overtime`, { ...d, userId: id, hours: h, hourlyRate: r, multiplier: m, totalAmount: h * r * m }); }, onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['/api/hr/employees', id, 'overtime'] }); setOvertimeDialogOpen(false); setOvertimeForm({ workDate: "", hours: "", hourlyRate: "", multiplier: "1.5", reason: "" }); toast({ title: tCommon('savedSuccessfully') }); } });
  const saveCashAdvanceMutation = useMutation({ mutationFn: async (d: typeof cashAdvanceForm) => apiRequest("POST", `/api/hr/employees/${id}/cash-advances`, { ...d, userId: id, amount: parseFloat(d.amount) || 0 }), onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['/api/hr/employees', id, 'cash-advances'] }); setCashAdvanceDialogOpen(false); setCashAdvanceForm({ requestDate: "", amount: "", reason: "", status: "pending" }); toast({ title: tCommon('savedSuccessfully') }); } });
  const saveLeaveRequestMutation = useMutation({ mutationFn: async (d: typeof leaveRequestForm) => { const s = new Date(d.startDate); const e = new Date(d.endDate); return apiRequest("POST", `/api/hr/employees/${id}/leave-requests`, { ...d, userId: id, totalDays: Math.ceil((e.getTime() - s.getTime()) / (1000 * 60 * 60 * 24)) + 1, status: "pending" }); }, onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['/api/hr/employees', id, 'leave-requests'] }); setLeaveRequestDialogOpen(false); setLeaveRequestForm({ leaveType: "annual", startDate: "", endDate: "", reason: "" }); toast({ title: tCommon('savedSuccessfully') }); } });
  ...
  const saveBusinessTripMutation = useMutation({ mutationFn: async (d: typeof businessTripForm) => { const s = new Date(d.startDate); const e = new Date(d.endDate); const dt = Math.ceil((e.getTime() - s.getTime()) / (1000 * 60 * 60 * 24)) + 1; const da = parseFloat(d.dailyAllowance) || 0; const tc = parseFloat(d.transportCost) || 0; const ac = parseFloat(d.accommodationCost) || 0; return apiRequest("POST", `/api/hr/employees/${id}/business-trips`, { ...d, userId: id, totalDays: dt, totalCost: da * dt + tc + ac, status: "planned" }); }, onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['/api/hr/employees', id, 'business-trips'] }); setBusinessTripDialogOpen(false); setBusinessTripForm({ destination: "", purpose: "", startDate: "", endDate: "", dailyAllowance: "", transportCost: "", accommodationCost: "" }); toast({ title: tCommon('savedSuccessfully') }); } });
