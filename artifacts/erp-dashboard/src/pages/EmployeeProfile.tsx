...
export default function EmployeeProfile() {
  ...
  const saveSalaryChangeMutation = useMutation({ mutationFn: async (d: typeof salaryForm) => { const p = parseFloat(d.previousSalary) || 0; const n = parseFloat(d.newSalary) || 0; return apiRequest("POST", `/api/employees/${id}/salary-history`, { ...d, userId: id, previousSalary: p, newSalary: n, changePercent: p > 0 ? ((n - p) / p * 100) : 0 }); }, onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['/api/employees', id, 'salary-history'] }); setSalaryDialogOpen(false); setSalaryForm({ effectiveDate: "", previousSalary: "", newSalary: "", changeType: "annual_review", notes: "" }); toast({ title: tCommon('savedSuccessfully') }); } });
  ...
  const saveSickLeaveMutation = useMutation({ mutationFn: async (d: typeof sickLeaveForm) => { const s = new Date(d.startDate); const e = new Date(d.endDate); return apiRequest("POST", `/api/employees/${id}/sick-leaves`, { ...d, userId: id, totalDays: Math.ceil((e.getTime() - s.getTime()) / (1000 * 60 * 60 * 24)) + 1, paymentPercent: parseInt(d.paymentPercent) || 100 }); }, onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['/api/employees', id, 'sick-leaves'] }); setSickLeaveDialogOpen(false); setSickLeaveForm({ startDate: "", endDate: "", diagnosis: "", hospitalName: "", doctorName: "", documentNumber: "", isPaid: true, paymentPercent: "100" }); toast({ title: tCommon('savedSuccessfully') }); } });
  ...
  const setPasswordMutation = useMutation({ mutationFn: async (p: { password: string }) => apiRequest("POST", `/api/employees/${id}/set-password`, { password: p.password }), onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['/api/employees', id] }); setPasswordDialogOpen(false); setPasswordForm({ password: "", confirmPassword: "" }); toast({ title: "Parol muvaffaqiyatli o'rnatildi" }); } });
  const updateRoleMutation = useMutation({ mutationFn: async (r: string) => apiRequest("PATCH", `/api/employees/${id}`, { role: r }), onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['/api/employees', id] }); toast({ title: "Rol muvaffaqiyatli yangilandi" }); } });
  const updateEmployeeMutation = useMutation({ mutationFn: async (d: typeof editForm) => apiRequest("PATCH", `/api/employees/${id}`, d), onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['/api/employees', id] }); queryClient.invalidateQueries({ queryKey: ['/api/employees'] }); setEditDialogOpen(false); toast({ title: "Xodim ma'lumotlari muvaffaqiyatli yangilandi" }); } });
