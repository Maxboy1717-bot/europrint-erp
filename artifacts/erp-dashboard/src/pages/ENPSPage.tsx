...
const STATUS_COLORS: Record<string, string> = { draft: "bg-slate-700", active: "bg-green-700", closed: "bg-slate-600" };
...
export default function ENPSPage() {
  ...
  const [createForm, setCreateForm] = useState({ title: "", period: `Q${Math.ceil((new Date().getMonth() + 1) / 3)}-${new Date().getFullYear()}`, created_by: 1 });
  ...
  return (
    <div className="p-6 space-y-6 bg-[#0f0f23] min-h-screen text-white">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-white">📊 eNPS — Xodimlar So'rovi</h1>
          <p className="text-slate-400 text-sm mt-1">Choraklik anonim xodim qoniqish so'rovi</p>
        </div>
      ...
      {showCreate && (
        <Card className="bg-slate-800/50 border-[#ff5d2e] max-w-lg">
          <CardHeader><CardTitle className="text-white">Yangi eNPS So'rov</CardTitle></CardHeader>
          <CardContent className="space-y-4">
            <div>
              <Label className="text-slate-300">Sarlavha</Label>
              <Input value={createForm.title} onChange={e => setCreateForm(f => ({ ...f, title: e.target.value }))}
                placeholder="Masalan: Q1 2026 Xodimlar so'rovi" className="bg-slate-700 border-slate-600 text-white mt-1" />
            </div>
            <div>
              <Label className="text-slate-300">Davr</Label>
              <Input value={createForm.period} onChange={e => setCreateForm(f => ({ ...f, period: e.target.value }))}
                placeholder="Q1-2026" className="bg-slate-700 border-slate-600 text-white mt-1" />
            </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="space-y-3">
          <h3 className="text-slate-400 text-sm font-medium">So'rovlar ({surveys?.length || 0})</h3>
          {surveys?.map((s) => (
            <Card key={s.id} onClick={() => setSelectedSurvey(s)}
              className={`cursor-pointer transition-all ${selectedSurvey?.id === s.id ? 'border-[#ff5d2e] bg-slate-700/50' : 'bg-slate-800/50 border-slate-700 hover:border-slate-600'}`}>
              <CardContent className="p-4">
                <div className="flex items-start justify-between mb-2">
                  <div className="font-semibold text-white text-sm">{s.title}</div>
                  <Badge className={`${STATUS_COLORS[s.status]} text-white text-xs`}>{STATUS_LABELS[s.status]}</Badge>
                </div>
                <div className="text-xs text-slate-400">{s.period}</div>
                <div className="text-xs text-slate-500 mt-1">{s.response_count || 0} javob</div>
                <div className="flex gap-1 mt-3">
                  ...
                  {s.status === 'active' && (
                    <Button size="sm" onClick={e => { e.stopPropagation(); close.mutate(s.id); }}
                      className="bg-slate-600 hover:bg-slate-500 text-white text-xs h-7">
                      ✅ Yopish
                    </Button>
                  )}
                </div>
          ...
          {(!surveys || surveys.length === 0) && (
            <div className="text-center py-8 text-slate-400 text-sm">Hali so'rov yo'q</div>
          )}
        </div>
        ...
        {selectedSurvey && (
          <div className="lg:col-span-2 space-y-4">
            {results ? (
              <>
                <Card className="bg-slate-800/50 border-slate-700">
                  <CardHeader><CardTitle className="text-white">📊 {selectedSurvey.title} — Natijalar</CardTitle></CardHeader>
                  <CardContent>
                    <div className="grid grid-cols-4 gap-4 mb-6">
                      {([
                        { label: "Jami javob", value: results.overall?.total_responses || 0, color: "text-white" },
                    ...
                    {results.overall != null && results.overall.enps_score !== null && (
                      <div className="text-center p-6 bg-slate-700/50 rounded-xl">
                        <div className="text-xs text-slate-400 mb-1">eNPS BALLI</div>
                        ...
                        <div className="text-xs text-slate-500 mt-1">O'rtacha qoniqish: {results.overall.avg_satisfaction}/5</div>
                      </div>
                    )}
                  </CardContent>
                </Card>
                ...
                {results.by_department?.length > 0 && (
                  <Card className="bg-slate-800/50 border-slate-700">
                    <CardHeader><CardTitle className="text-white text-base">Bo'limlar bo'yicha</CardTitle></CardHeader>
                    <CardContent>
                      <div className="space-y-2">
                        {(Array.isArray(results.by_department) ? results.by_department : []).map((d) => (
                          <div key={d.department_name} className="flex items-center gap-4">
                            <div className="w-36 text-slate-300 text-sm truncate">{d.department_name || "Noma'lum"}</div>
                            ...
                            {d.needs_attention && <Badge className="bg-red-700/50 text-red-300 text-xs">⚠️ E'tibor</Badge>}
                          </div>
                        ))}
                      </div>
                    </CardContent>
                ...
                {results.feedback_comments?.length > 0 && (
                  <Card className="bg-slate-800/50 border-slate-700">
                    <CardHeader><CardTitle className="text-white text-base">💬 Fikrlar (anonim)</CardTitle></CardHeader>
                    <CardContent>
                      <div className="space-y-2">
                        {(Array.isArray(results.feedback_comments) ? results.feedback_comments : []).map((c, i) => (
                          <div key={`k-${i}`} className="p-3 bg-slate-700/30 rounded-lg">
                            <p className="text-slate-300 text-sm italic">"{c.feedback}"</p>
                            <div className="text-xs text-slate-500 mt-1">{new Date(c.submitted_at).toLocaleDateString()}</div>
                          </div>
                        ))}
                      </div>
                    </CardContent>
            ...
            {selectedSurvey.status === 'active' && (
              <Card className="bg-slate-800/50 border-slate-700">
                <CardHeader><CardTitle className="text-white text-base">📝 Javob berish (Anonim)</CardTitle></CardHeader>
                <CardContent className="space-y-4">
                  <div>
                    <Label className="text-slate-300 text-sm">EuroPrint'da ishlashni tavsiya qilasizmi? (0-10)</Label>
                      ...
                      {Array.from({ length: 11 }, (_, i) => (
                        <button key={`k-${i}`} onClick={() => setResponseForm(f => ({ ...f, nps_score: i }))}
                          className={`w-9 h-9 rounded text-sm font-bold transition-all ${responseForm.nps_score === i
                            ? (i >= 9 ? 'bg-green-600 text-white' : i >= 7 ? 'bg-yellow-600 text-white' : 'bg-red-600 text-white')
                  ...
                  <div>
                    <Label className="text-slate-300 text-sm">Qoniqish darajasi (1-5)</Label>
                    <div className="flex gap-2 mt-2">
                      {([1, 2, 3, 4, 5]).map(n => (
                        <button key={n} onClick={() => setResponseForm(f => ({ ...f, satisfaction_score: n }))}
                          className={`w-10 h-10 rounded text-lg transition-all ${responseForm.satisfaction_score === n ? 'bg-[#ff5d2e]' : 'bg-slate-700 hover:bg-slate-600'}`}>
                          {["😞", "😕", "😐", "😊", "😄"][n - 1]}
                        </button>
                      ))}
                    </div>
                  ...
                  <div>
                    <Label className="text-slate-300 text-sm">Nima yaxshilanishi kerak? (ixtiyoriy)</Label>
                    <Textarea value={responseForm.feedback} onChange={e => setResponseForm(f => ({ ...f, feedback: e.target.value }))}
                      placeholder="Fikringizni baham ko'ring..." className="bg-slate-700 border-slate-600 text-white mt-1" />
                  </div>
                  <Button onClick={() => respond.mutate({ id: selectedSurvey!.id!, data: responseForm })}
                    disabled={respond.isPending}
                    className="bg-[#ff5d2e] hover:bg-[#e04d1e] text-white w-full">
