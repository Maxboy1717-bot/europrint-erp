...
import { 
  ...
  Activity, PieChart, CheckSquare, Clock, Phone, Users, Zap, FileText, TrendingDown
} from "lucide-react";
...
interface CrmActivity { id: number; type: string; subject: string; dueDate: string | null; isDone: boolean; entityType: string | null; entityId: number | null; }

const ACTIVITY_TYPE_ICONS: Record<string, typeof Phone> = { call: Phone, meeting: Users, email: Activity, follow_up: CheckSquare, other: Activity };
const ACTIVITY_TYPE_LABELS: Record<string, string> = { call: "Qo'ng'iroq", meeting: "Uchrashuv", email: "Email", follow_up: "Kuzatuv", other: "Boshqa" };
...
const PRIORITY_LABELS: Record<string, string> = { high: "Yuqori", medium: "O'rta", low: "Past" };
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-4xl font-light tracking-tight text-on-surface">
              Umumiy <span className="font-bold text-primary">Ko'rinish</span>
            </h1>
            <p className="text-on-surface-variant mt-1">Savdo tizimining barcha ko'rsatkichlari (Real-time)</p>
          </div>
        </div>
        <div className="flex items-center gap-4">
          <div className="bg-surface-container-lowest rounded-lg p-5">
            <p className="text-xs font-semibold uppercase tracking-wider text-on-surface-variant">Oylik buyurtmalar</p>
            <p className="text-4xl font-bold tracking-tight text-on-surface mt-1">{overview?.monthlyOrders || 0} ta</p>
          </div>
          <div className="bg-surface-container-lowest rounded-lg p-5">
            <p className="text-xs font-semibold uppercase tracking-wider text-on-surface-variant">Oylik tushum</p>
            <p className="text-4xl font-bold tracking-tight text-on-surface mt-1">{fmt(overview?.monthlyCollected || 0)}</p>
          </div>
        ...
        <>
          <div className="grid grid-cols-2 md:grid-cols-3 gap-6 mb-8">
            {(Array.isArray(stats) ? stats : []).map(s => (
              <div key={s.label} className="bg-surface-container-lowest rounded-lg p-5">
                <div className="flex items-center justify-between mb-2">
                  <p className="text-xs font-semibold uppercase tracking-wider text-on-surface-variant">{s.label}</p>
                  <s.icon className={cn("w-4 h-4", s.color)} />
                </div>
                <p className="text-4xl font-bold tracking-tight text-on-surface mt-1">{s.value}</p>
          ...
          {managerActions && managerActions.actionList && managerActions.actionList.length > 0 && (
            <div className="bg-surface-container-lowest rounded-xl p-6 mb-6">
              <div className="flex items-center justify-between mb-4 flex-wrap gap-2">
                <h2 className="text-xs font-semibold uppercase tracking-wider text-on-surface-variant flex items-center gap-2">
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-2 mt-1">
                          <Badge variant="secondary" className="text-[10px] no-default-hover-elevate px-1.5 py-0.5">
              ...
              <div className="grid grid-cols-3 gap-4 mt-4 pt-4 border-t border-outline-variant">
                <div className="text-center">
                  <p className="text-2xl font-bold text-red-600">{fmt(managerActions.overduePayments?.amount || 0)}</p>
                  <p className="text-xs text-on-surface-variant mt-0.5">Muddati o'tgan qarz</p>
                </div>
                <div className="text-center">
                  <p className="text-2xl font-bold text-amber-600">{managerActions.delayedOrders?.total || 0} ta</p>
                  <p className="text-xs text-on-surface-variant mt-0.5">Kechikkan buyurtma</p>
                </div>
                <div className="text-center">
                  <p className="text-2xl font-bold text-blue-600">{managerActions.pendingProposals?.total || 0} ta</p>
                  <p className="text-xs text-on-surface-variant mt-0.5">Kutilayotgan taklif</p>
                </div>
              </div>
            </div>
          )}
          ...
          <div className="bg-surface-container-lowest rounded-xl p-6 mb-6">
            <div className="flex items-center justify-between mb-4">
              ...
              <div className="flex items-center gap-2">
                <Badge variant="secondary" className="no-default-hover-elevate">{(Array.isArray(todayActivities) ? todayActivities : []).filter(a => !a.isDone).length} aktiv</Badge>
                <Badge className="bg-green-100 text-green-800 no-default-hover-elevate">{(Array.isArray(todayActivities) ? todayActivities : []).filter(a => a.isDone).length} bajarildi</Badge>
              </div>
            </div>
            {todayActivities.length === 0 ? (
              <p className="text-sm text-on-surface-variant text-center py-4">Bugun uchun faoliyat rejalashtirilmagan</p>
            ) : (
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                {(Array.isArray(todayActivities) ? todayActivities : []).slice(0, 6).map(act => {
                  ...
                  return (
                    <div key={act.id} data-testid={`card-dashboard-activity-${act.id}`}
                      className={cn("flex items-start gap-3 p-3 rounded-lg", act.isDone ? "opacity-60 bg-surface-container" : isOverdue ? "bg-red-50" : "bg-surface-container-low")}>
                      <Icon className={cn("w-4 h-4 shrink-0 mt-0.5", isOverdue ? "text-red-500" : "text-primary")} />
                      <div className="flex-1 min-w-0">
                        <p className="text-sm font-medium text-on-surface truncate">{act.subject}</p>
                        <p className="text-xs text-on-surface-variant">{ACTIVITY_TYPE_LABELS[act.type] || act.type}</p>
                        {act.dueDate && (
                          <p className={cn("text-xs flex items-center gap-1 mt-0.5", isOverdue ? "text-red-600 font-medium" : "text-on-surface-variant")}>
                            <Clock className="w-3 h-3" /> {act.dueDate.slice(11, 16)}
                          </p>
                        )}
                      </div>
                      {act.isDone && <CheckSquare className="w-4 h-4 text-green-500 shrink-0" />}
                    </div>
                  );
                })}
              </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="bg-surface-container-lowest rounded-xl p-6">
              <h2 className="text-xs font-semibold uppercase tracking-wider text-on-surface-variant flex items-center gap-2 mb-4">
                <Activity className="w-4 h-4 text-primary" />
                Lead Funnel (Sotuv voronkasi)
              </h2>
              <div className="flex flex-wrap gap-3">
                {(Array.isArray(funnelOrder) ? funnelOrder : []).map(st => (
                  <div key={st} className="flex items-center gap-2 p-2 bg-surface-container-low rounded-lg">
                    <span className={cn(
                    ...
                    <span className="font-bold text-sm text-on-surface">{funnel[st] || 0}</span>
                    {st !== "frozen" && <ArrowRight className="w-3 h-3 text-on-surface-variant" />}
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="bg-surface-container-lowest rounded-xl p-6">
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  <span className="text-sm text-on-surface">Yutilgan leadlar (Conversion Rate)</span>
