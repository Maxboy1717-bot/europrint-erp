...
export default function DirectorExtended() {
  ...
  const kpiList: KpiListItem[] = [
    { name: t("kpiSavdoHajmi"), value: dashboardData ? `${dashboardData.orders?.month || 0} ta` : "...", target: "100 ta", pct: dashboardData ? Math.min(Math.round(dashboardData.orders?.month || 0), 100) : 0, trend: "up" },
    { name: t("kpiBuyurtmaBajarish"), value: dashboardData ? `${Math.round((dashboardData.orders?.completed || 0) / Math.max(dashboardData.orders?.month || 1, 1) * 100)}%` : "...", target: "95%", pct: dashboardData ? Math.round((dashboardData.orders?.completed || 0) / Math.max(dashboardData.orders?.month || 1, 1) * 100) : 0, trend: "up" },
    { name: t("kpiOeeOrtacha"), value: dashboardData?.production?.oee ? `${dashboardData.production.oee}%` : t("malumotYoq"), target: "85%", pct: dashboardData?.production?.oee || 0, trend: "down" },
    { name: t("kpiKechikkan"), value: dashboardData ? `${dashboardData.orders?.overdue || 0} ta` : "...", target: "0 ta", pct: dashboardData?.orders?.overdue === 0 ? 100 : 0, trend: (dashboardData?.orders?.overdue || 0) === 0 ? "up" : "down" },
    { name: t("kpiDavomad"), value: dashboardData ? `${dashboardData.hr?.attendanceRate || 0}%` : "...", target: "97%", pct: dashboardData?.hr?.attendanceRate || 0, trend: "up" },
    { name: t("kpiIotSignallar"), value: dashboardData ? `${dashboardData.alerts?.iot || 0} ta` : "...", target: "0", pct: dashboardData?.alerts?.iot === 0 ? 100 : 0, trend: (dashboardData?.alerts?.iot || 0) === 0 ? "up" : "down" },
  ];
      ...
      />
          {criticalAlerts.length > 0 && (
            <Badge className="bg-red-100 text-red-800 rounded-full px-2.5 py-0.5 text-xs font-semibold">
              <ShieldAlert className="h-3 w-3 mr-1" />{criticalAlerts.length} {t("kritikBadge")}
            </Badge>
          )}
        </div>
        <Button
