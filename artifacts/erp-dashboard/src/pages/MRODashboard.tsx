...
export default function MRODashboard() {
  ...
  return (
    <div className="flex flex-col h-full p-5 lg:p-6 gap-5" data-testid="page-mro-dashboard">
      <div>
        <EPPageHeader
        breadcrumb={<>Dashboard · <b className="text-foreground">Xo'jalik Boshqaruvi</b></>}
        title="Xo'jalik Boshqaruvi"
        subtitle="Xo'jalik materiallari, ta'mirlash va texnik xizmat"
      />
      </div>

      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-card rounded-lg p-5"><p className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">Jami buyumlar</p><p className="text-4xl font-bold tracking-tight text-foreground mt-1">{stats?.items?.totalItems || 0}</p></div>
        <div className="bg-card rounded-lg p-5"><p className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">Kam zaxira</p><p className="text-4xl font-bold tracking-tight text-[var(--ep-red)] mt-1">{lowStockItems}</p></div>
        <div className="bg-card rounded-lg p-5"><p className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">Kutilayotgan so'rovlar</p><p className="text-4xl font-bold tracking-tight text-[var(--ep-yellow)] mt-1">{pendingRequests}</p></div>
        <div className="bg-card rounded-lg p-5"><p className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">Rejalashtirilgan TA</p><p className="text-4xl font-bold tracking-tight text-primary mt-1">{scheduledMaintenance}</p></div>
      </div>

      <Tabs defaultValue="items" className="w-full">
        <TabsList className="bg-muted/40 p-1 rounded-lg flex-wrap h-auto">
          <TabsTrigger value="items"     data-testid="tab-items"     className="data-[state=active]:bg-card">Buyumlar</TabsTrigger>
          <TabsTrigger value="requests"  data-testid="tab-requests"  className="data-[state=active]:bg-card">So'rovlar</TabsTrigger>
          <TabsTrigger value="equipment" data-testid="tab-equipment" className="data-[state=active]:bg-card">Uskunalar</TabsTrigger>
          <TabsTrigger value="utilities" data-testid="tab-utilities" className="data-[state=active]:bg-card">Kommunal</TabsTrigger>
          <TabsTrigger value="building"  data-testid="tab-building"  className="data-[state=active]:bg-card">Bino/Xonalar</TabsTrigger>
          <TabsTrigger value="cleaning"  data-testid="tab-cleaning"  className="data-[state=active]:bg-card">Tozalash</TabsTrigger>
          <TabsTrigger value="uniforms"  data-testid="tab-uniforms"  className="data-[state=active]:bg-card">Forma</TabsTrigger>
        </TabsList>
        ...
        <TabsContent value="building" className="mt-6">
          <div className="bg-card rounded-xl p-6">
            <div className="flex items-center gap-2 mb-6">
              ...
              <h2 className="text-xl font-bold text-foreground">Bino va Xonalar Inventari</h2>
            </div>
        ...
        <TabsContent value="cleaning" className="mt-6">
          <div className="bg-card rounded-xl p-6">
            <div className="flex items-center gap-2 mb-6">
              ...
              <h2 className="text-xl font-bold text-foreground">Tozalash Jadvali</h2>
            </div>
        ...
        <TabsContent value="uniforms" className="mt-6">
          <div className="bg-card rounded-xl p-6">
            <div className="flex items-center gap-2 mb-6">
              ...
              <h2 className="text-xl font-bold text-foreground">Forma va PPE Inventari</h2>
            </div>
