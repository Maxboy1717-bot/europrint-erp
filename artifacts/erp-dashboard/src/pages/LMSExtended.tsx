    ...
    <div className="flex flex-col h-full bg-background">
      <div className="border-b border-border px-6 py-4 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <EPPageHeader
        breadcrumb={<>Dashboard · <b className="text-foreground">LMS Kengaytirilgan</b></>}
        title="LMS Kengaytirilgan"
      />
        ...
        <div className="flex-1 overflow-auto p-6 bg-background">
          <TabsContent value="author" className="mt-0 space-y-6 outline-none">
            <div className="flex items-center justify-between">
              <h2 className="text-xl font-bold text-foreground">Kurslar Boshqaruvi</h2>
              <div className="flex gap-2">
                <Button variant="outline" size="sm" onClick={() => refetchCourses()} className="bg-muted/60 text-foreground hover:bg-muted border-none rounded-lg px-4 font-medium"><RefreshCw className="h-3.5 w-3.5 mr-1.5" />Yangilash</Button>
                <Button size="sm" onClick={() => setShowCourseDialog(true)} data-testid="button-create-course" className="bg-primary text-white rounded-lg px-5 py-2 text-sm font-semibold shadow-none"><Plus className="h-3.5 w-3.5 mr-1.5" />Yangi Kurs</Button>
              </div>
            </div>
            ...
            <Card className="bg-card border-border shadow-none">
              <CardContent className="p-0">
                <div className="ep-table-scroll"><Table>
                  ...
                  <TableBody>
                    {coursesLoading ? <TableRow className="border-none"><TableCell colSpan={5} className="text-center py-6 text-[13px] text-muted-foreground">Yuklanmoqda...</TableCell></TableRow>
                    : courses.length === 0 ? <TableRow className="border-none"><TableCell colSpan={5} className="text-center py-8 text-[13px] text-muted-foreground"><BookOpen className="h-8 w-8 mx-auto mb-2 opacity-30" />Kurslar yo'q</TableCell></TableRow>
            ...
            <Dialog open={showCourseDialog} onOpenChange={setShowCourseDialog}>
              <DialogContent className="bg-card border-border p-6">
                <DialogHeader><DialogTitle className="text-foreground">Yangi Kurs Yaratish</DialogTitle></DialogHeader>
                <form onSubmit={courseForm.handleSubmit(d => createCourse.mutate(d))} className="space-y-4">
                  <div className="space-y-2"><Label className="text-foreground">Kurs nomi</Label><Input {...courseForm.register("title")} placeholder="Kurs nomini kiriting" data-testid="input-course-title" className="bg-muted/40 border-border" /></div>
                  <div className="space-y-2"><Label className="text-foreground">Tavsif</Label><Input {...courseForm.register("description")} placeholder="Qisqa tavsif" className="bg-muted/40 border-border" /></div>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    ...
                    <div className="space-y-2"><Label className="text-foreground">Daraja</Label><Controller control={courseForm.control} name="level" render={({ field }) => (<Select value={field.value} onValueChange={field.onChange}><SelectTrigger className="bg-muted/40 border-border h-9"><SelectValue /></SelectTrigger><SelectContent className="bg-card border-border"><SelectItem value="beginner">Boshlang'ich</SelectItem><SelectItem value="intermediate">O'rta</SelectItem><SelectItem value="advanced">Yuqori</SelectItem></SelectContent></Select>)} /></div>
                  </div>
                  <DialogFooter>
                    <Button variant="outline" type="button" onClick={() => setShowCourseDialog(false)} className="bg-muted/60 text-foreground hover:bg-muted border-none rounded-lg font-medium">Bekor</Button>
                    <Button type="submit" disabled={createCourse.isPending} className="bg-primary text-white rounded-lg px-5 py-2 text-sm font-semibold shadow-none">Yaratish</Button>
                  </DialogFooter>
                </form>
              </DialogContent>
            </Dialog>
          ...
          <TabsContent value="tests" className="mt-0 space-y-6 outline-none">
            <div className="flex items-center justify-between">
              ...
              <div className="flex gap-2">
                <Button variant="outline" size="sm" onClick={() => refetchTests()} className="bg-muted/60 text-foreground hover:bg-muted border-none rounded-lg px-4 font-medium"><RefreshCw className="h-3.5 w-3.5 mr-1.5" />Yangilash</Button>
                <Button size="sm" onClick={() => setShowTestDialog(true)} data-testid="button-create-test" className="bg-primary text-white rounded-lg px-5 py-2 text-sm font-semibold shadow-none"><Plus className="h-3.5 w-3.5 mr-1.5" />Test Yaratish</Button>
              </div>
            </div>
            ...
            <Card className="bg-card border-border shadow-none">
              <CardContent className="p-0">
                <div className="ep-table-scroll"><Table>
                  ...
                  <TableBody>
                    {testsLoading ? <TableRow className="border-none"><TableCell colSpan={5} className="text-center py-6 text-[13px] text-muted-foreground">Yuklanmoqda...</TableCell></TableRow>
                    : tests.length === 0 ? <TableRow className="border-none"><TableCell colSpan={5} className="text-center py-8 text-[13px] text-muted-foreground"><ClipboardList className="h-8 w-8 mx-auto mb-2 opacity-30" />Testlar yo'q</TableCell></TableRow>
            ...
            <Dialog open={showTestDialog} onOpenChange={setShowTestDialog}>
              <DialogContent className="bg-card border-border p-6">
                <DialogHeader><DialogTitle className="text-foreground">Yangi Test Yaratish</DialogTitle></DialogHeader>
                <form onSubmit={testForm.handleSubmit(d => createTest.mutate(d))} className="space-y-4">
                  <div className="space-y-2"><Label className="text-foreground">Test nomi</Label><Input {...testForm.register("title")} placeholder="Test nomini kiriting" data-testid="input-test-title" className="bg-muted/40 border-border" /></div>
                  <div className="space-y-2"><Label className="text-foreground">Kurs</Label><Controller control={testForm.control} name="courseId" render={({ field }) => (<Select value={field.value} onValueChange={field.onChange}><SelectTrigger className="bg-muted/40 border-border h-9"><SelectValue placeholder="Kursni tanlang" /></SelectTrigger><SelectContent className="bg-card border-border">{(Array.isArray(courses) ? courses : []).map((c) => <SelectItem key={c.id} value={String(c.id)}>{c.title || c.name}</SelectItem>)}</SelectContent></Select>)} /></div>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  ...
                  <DialogFooter>
                    <Button variant="outline" type="button" onClick={() => setShowTestDialog(false)} className="bg-muted/60 text-foreground hover:bg-muted border-none rounded-lg font-medium">Bekor</Button>
                    <Button type="submit" disabled={createTest.isPending} className="bg-primary text-white rounded-lg px-5 py-2 text-sm font-semibold shadow-none">Yaratish</Button>
                  </DialogFooter>
                </form>
              </DialogContent>
            </Dialog>
          ...
          <TabsContent value="cert" className="mt-0 space-y-6 outline-none">
            ...
            <div className="p-4 rounded-lg bg-primary/10/10 border border-primary/20 text-sm text-foreground flex items-start gap-3"><Zap className="h-4 w-4 text-primary mt-0.5" /><p>MES tizimiga kirish uchun stanoq sertifikati majburiy. Sertifikatsiz operator MES da o'sha stanoqda ishlay olmaydi.</p></div>
            <Card className="bg-card border-border shadow-none">
              <CardContent className="p-0"><div className="ep-table-scroll"><Table><TableHeader className="sticky top-0 z-10 bg-card"><TableRow className="border-none hover:bg-transparent">{(["Operator","Daraja","Tasdiqlangan sertifikatlar","Kutilayotgan"]).map((h,i) => <TableHead key={h} className={`bg-muted/60 text-xs font-semibold uppercase tracking-wider text-muted-foreground py-3 px-6 ${i===0?"rounded-l-lg":""} ${i===3?"rounded-r-lg":""}`}>{h}</TableHead>)}</TableRow></TableHeader><TableBody><TableRow className="border-none"><TableCell colSpan={4} className="text-center py-10 text-[13px] text-muted-foreground"><Award className="h-10 w-10 mx-auto mb-3 opacity-20" /><p className="font-medium">Sertifikat ma'lumotlari mavjud emas</p><p className="text-sm mt-1 opacity-70">Operator sertifikatlari MES tizimi orqali qo'shiladi</p></TableCell></TableRow></TableBody></Table></div></CardContent>
            </Card>
          </TabsContent>
