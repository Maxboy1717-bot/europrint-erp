      ...
      <SheetTrigger asChild>
        {trigger ?? (
          <Button
            variant="ghost"
            size="sm"
            className="h-6 text-[10px] text-violet-400 hover:text-violet-300 hover:bg-violet-500/10 gap-1 px-1"
            ...
            <>
              ...
              <div className="rounded-lg border border-red-200">
                <button
                  ...
                  className="w-full flex items-center justify-between px-3 py-2.5 rounded-t-lg bg-red-50 text-sm font-semibold text-red-700"
                >
                  <span className="flex items-center gap-2">
                  ...
                  <div className="bg-white rounded-b-lg p-3 space-y-1.5">
                    {(Array.isArray(IMPORTANT_RULES) ? IMPORTANT_RULES : []).map((rule, i) => (
                      <div key={`k-${i}`} className="flex items-start gap-2">
                        <span className="w-4 h-4 rounded-full bg-red-100 text-red-600 flex items-center justify-center text-[9px] font-bold shrink-0 mt-0.5">
                          {i + 1}
                        </span>
          ...
          {activeTab === "cold" && (
            <div className="space-y-3">
              <div className="bg-violet-50 border border-violet-200 rounded-lg p-3">
                <p className="text-xs text-violet-700 font-medium mb-1">Sovuq qo'ng'iroq nima?</p>
                ...
                {(Array.isArray(COLD_CALL_SCRIPT) ? COLD_CALL_SCRIPT : []).map((item, i) => (
                  <div key={`k-${i}`} className="bg-white border border-slate-200 rounded-lg p-3">
                    <div className="flex items-center gap-2 mb-2">
                      <span className="w-5 h-5 rounded-full bg-violet-500 text-white flex items-center justify-center text-[10px] font-bold shrink-0">
                        {i + 1}
                      </span>
              ...
              <div className="bg-amber-50 border border-amber-200 rounded-lg p-3">
                <p className="text-[10px] font-semibold text-amber-700 mb-1">Eslatma:</p>
