...
export function InterviewDetailView({interview, onClose }: InterviewDetailViewProps) {
  ...
  return (
    <ModulePage module="ai" title="AI Intervyu" subtitle="Sun'iy intellekt yordamida avtomatik intervyu o'tkazish" icon={<Mic className="h-5 w-5" />}
      actions={<Button variant="outline" data-testid="button-close-detail" onClick={onClose}><X className="h-4 w-4 mr-2" />Yopish</Button>}
    >
      <Card data-testid="card-interview-detail">
        ...
        <CardContent className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="space-y-2"><p className="text-sm text-muted-foreground">Nomzod ID</p><p className="font-medium" data-testid="text-detail-candidate">{interview.candidateId}</p></div>
            {interview.candidateName && <div className="space-y-2"><p className="text-sm text-muted-foreground">Nomzod ismi</p><p className="font-medium">{interview.candidateName}</p></div>}
            {interview.jobTitle && <div className="space-y-2"><p className="text-sm text-muted-foreground">Lavozim</p><p className="font-medium">{interview.jobTitle}</p></div>}
            <div className="space-y-2"><p className="text-sm text-muted-foreground">Til</p><p className="font-medium">{interview.language === "uz" ? "O'zbek" : "Rus"}</p></div>
            {interview.provider && <div className="space-y-2"><p className="text-sm text-muted-foreground">Provider</p><div>{getProviderBadge(interview.provider)}</div></div>}
            {interview.scheduledAt && <div className="space-y-2"><p className="text-sm text-muted-foreground">Rejalashtirilgan</p><p className="font-medium">{formatDateTime(interview.scheduledAt)}</p></div>}
            {interview.startedAt && <div className="space-y-2"><p className="text-sm text-muted-foreground">Boshlangan</p><p className="font-medium">{formatDateTime(interview.startedAt)}</p></div>}
            {interview.completedAt && <div className="space-y-2"><p className="text-sm text-muted-foreground">Tugallangan</p><p className="font-medium">{formatDateTime(interview.completedAt)}</p></div>}
            {interview.duration && <div className="space-y-2"><p className="text-sm text-muted-foreground">Davomiyligi</p><p className="font-medium">{formatDuration(interview.duration)}</p></div>}
          </div>
          ...
          {interview.transcript && interview.transcript.length > 0 && (
            <div className="space-y-3">
              <h3 className="font-semibold flex items-center gap-2"><MessageSquare className="h-4 w-4" />Transkript</h3>
          ...
          {interview.evaluation && (
            <div className="space-y-3">
              <h3 className="font-semibold flex items-center gap-2"><Star className="h-4 w-4" />Baholash</h3>
              <Card>
                <CardContent className="p-4 space-y-4">
                  {interview.evaluation.overallScore !== undefined && (
                    <div className="flex items-center gap-3"><span className="text-sm text-muted-foreground">Umumiy ball:</span><Badge data-testid="badge-overall-score">{interview.evaluation.overallScore}/100</Badge></div>
                  )}
                  {interview.evaluation.strengths && interview.evaluation.strengths.length > 0 && (
                    <div>
                      <p className="text-sm font-medium mb-2 text-[var(--ep-green)] dark:text-green-400">Kuchli tomonlar:</p>
                  ...
                  {interview.evaluation.weaknesses && interview.evaluation.weaknesses.length > 0 && (
                    <div>
                      <p className="text-sm font-medium mb-2 text-[var(--ep-red)] dark:text-red-400">Zaif tomonlar:</p>
                  ...
                  {interview.evaluation.recommendation && (
                    <div><p className="text-sm font-medium mb-1">Tavsiya:</p><p className="text-sm text-muted-foreground" data-testid="text-recommendation">{interview.evaluation.recommendation}</p></div>
                  )}
                </CardContent>
              </Card>
            </div>
...
export function AddQuestionForm({ qForm, setQForm, onSubmit, isPending, onCancel }: AddQuestionFormProps) {
  return (
    <Card data-testid="card-add-question">
      <CardHeader><CardTitle className="text-base">Yangi savol qo'shish</CardTitle></CardHeader>
      <CardContent>
...
export function CreateInterviewForm({ form, onSubmit, isPending, onCancel }: CreateInterviewFormProps) {
  return (
    <Card className="mb-6" data-testid="card-create-form">
      <CardHeader><CardTitle className="text-[14px] font-semibold">Yangi intervyu yaratish</CardTitle></CardHeader>
      <CardContent>
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="space-y-1">
          <Label htmlFor="candidateId">Nomzod ID</Label>
              <Input id="candidateId" {...form.register("candidateId")} placeholder="Nomzod ID kiriting" data-testid="input-candidate-id" />
          ...
          <Label htmlFor="jobTitle">Lavozim</Label>
              <Input id="jobTitle" {...form.register("jobTitle")} placeholder="Lavozim nomini kiriting" data-testid="input-job-title" />
          ...
          <Label htmlFor="language">Til</Label>
              <Controller control={form.control} name="language" render={({ field }) => (
                <Select value={field.value} onValueChange={field.onChange}>
                  <SelectTrigger data-testid="select-language" className="h-9"><SelectValue placeholder="Tilni tanlang" /></SelectTrigger>
                  <SelectContent>
          ...
          <div className="flex gap-3 flex-wrap">
            ...
            <Button type="button" variant="outline" onClick={onCancel} data-testid="button-cancel-create">Bekor qilish</Button>
          </div>
        </form>
      </CardContent>
    </Card>
