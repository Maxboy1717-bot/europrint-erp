    ...
    <Card className="bg-card border-none shadow-sm">
      <CardHeader>
        <CardTitle className="flex items-center gap-2">
          ...
          AI Email Shablon Generatori
        </CardTitle>
        <CardDescription>Kontekst asosida professional email shablonlar yaratish</CardDescription>
      </CardHeader>
      <CardContent className="space-y-4">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="space-y-1">
          <Label>Bitim tanlash</Label>
            <Select onValueChange={(val) => onSelectDeal(deals.find(d => d.id === Number(val)) || null)}>
              <SelectTrigger><SelectValue placeholder="Bitim tanlang..." /></SelectTrigger>
              <SelectContent>
          ...
          <Label>Ton</Label>
            <Select value={emailTone} onValueChange={onEmailToneChange}>
              ...
              <SelectContent>
                ...
                <SelectItem value="persuasive">Ishontiruvchi</SelectItem>
              </SelectContent>
            </Select>
          </div>
        </div>
        <div className="space-y-1">
          <Label>Qo'shimcha kontekst</Label>
          <Textarea
            placeholder="Email maqsadi, qo'shimcha ma'lumotlar..."
            value={emailContext}
        ...
        {emailResult && (
          <div className="mt-4 p-4 rounded-lg bg-muted/60 space-y-3 border border-primary/20">
            {emailResult.subject && (
              <div>
                <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">Mavzu</p>
                <p className="font-medium mt-1">{emailResult.subject}</p>
              </div>
            )}
            {emailResult.template && (
              <div>
                <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">Matn</p>
...
export function ActionsTabContent({ deals, dealsLoading, aiResults, actionPending, actionVariable, onGetAction }: ActionsTabContentProps) {
  return (
    <Card className="bg-card border-none shadow-sm">
      <CardHeader>
        <CardTitle className="flex items-center gap-2"><Zap className="h-4 w-4 text-primary" />Keyingi eng yaxshi harakat</CardTitle>
      ...
      <CardContent>
        {dealsLoading ? (
          ...
          <div className="text-center py-8 text-[13px] text-muted-foreground"><Zap className="h-10 w-10 mx-auto mb-3 opacity-40" /><p>Bitimlar topilmadi</p></div>
        ) : (
                ...
                <div key={deal.id} className="p-4 rounded-lg bg-muted/60 space-y-2">
                  ...
                  {result?.nextAction && (
                    <div className="flex items-start gap-2 p-3 rounded-md bg-primary/5 border border-primary/20">
                      ...
                      <div>
                        <p className="text-sm font-medium text-primary">Tavsiya</p>
