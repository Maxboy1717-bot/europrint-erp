    ...
    <div className="p-6 space-y-6 bg-background min-h-screen">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          <Select value={period} onValueChange={setPeriod}>
            <SelectTrigger className="w-36 bg-input border-border h-9">
              <SelectValue />
            </SelectTrigger>
            <SelectContent>
          ...
          <Dialog open={awardBadgeOpen} onOpenChange={setAwardBadgeOpen}>
            ...
            <DialogContent className="max-w-md p-6">
              ...
              <div className="space-y-4 mt-2">
                ...
                <Button
                  onClick={() => awardBadgeMut.mutate({ ...badgeForm, awarded_by: currentUserId })}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
        {([
          ...
          { label: "Badge turlari", value: (catalog || []).length, icon: Award, color: "text-primary" },
        ]).map(s => (
      ...
      <Tabs defaultValue="leaderboard">
          ...
          <Card className="bg-card border-border">
            <CardHeader>
              <CardTitle className="text-foreground flex items-center gap-2">
                ...
                <Badge className="bg-primary text-white ml-2">{lb.length} xodim</Badge>
              </CardTitle>
            </CardHeader>
            <CardContent>
              {lbLoading ? (
                    ...
                    return (
                      <div key={emp.employee_id}
                        className={`flex items-center gap-4 p-3 rounded-lg transition-colors ${
                          ...
                          idx < 3 ? 'bg-muted' : 'bg-card hover:bg-muted'
                        }`}>
                        <div className={`w-10 h-10 rounded-full flex items-center justify-center font-bold text-lg flex-shrink-0 ${
                          idx === 0 ? 'bg-[var(--ep-yellow)] text-black' : idx === 1 ? 'bg-slate-400 text-black' :
                          idx === 2 ? 'bg-[var(--ep-yellow)] text-white' : 'bg-muted text-muted-foreground'
                        }`}>
                          {idx === 0 ? '🥇' : idx === 1 ? '🥈' : idx === 2 ? '🥉' : idx + 1}
                        </div>
                        <div className="flex-1 min-w-0">
                          <div className="font-semibold text-foreground truncate">{emp.first_name} {emp.last_name}</div>
                          <div className="text-xs text-muted-foreground truncate">{emp.department_name || emp.employee_code}</div>
                          <div className="h-1.5 bg-muted rounded-full mt-1.5 overflow-hidden">
                            <div className="h-full from-primary to-amber-400 rounded-full"
                        ...
                        <div className="text-right flex-shrink-0">
                          <div className="text-primary font-bold text-xl">
                            {pts.toLocaleString()}
                            <span className="text-xs text-muted-foreground ml-1">ball</span>
                          </div>
                        ...
                        <Button size="sm" variant="ghost"
                          className="text-muted-foreground hover:text-foreground hover:bg-muted flex-shrink-0 h-9 w-8 p-0"
        ...
        <TabsContent value="badges" className="mt-4">
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
            {(catalog || []).map((badge) => (
              <Card key={badge.code} className="bg-card border-border hover:border-primary/50 transition-colors group">
                <CardContent className="p-4 text-center">
                  ...
                  <div className="text-xs text-muted-foreground mt-0.5">{badge.name_ru}</div>
                  <div className="flex justify-center gap-1 mt-2 flex-wrap">
                    {badge.point_value > 0 && (
                      <Badge className="bg-primary/10 text-primary border-0 text-xs">+{badge.point_value} ball</Badge>
                    )}
                    {badge.is_auto_award && (
                      <EPStatusPill tone="info">{t('auto')}</EPStatusPill>
                    )}
                  </div>
                  <p className="text-xs text-muted-foreground mt-2 line-clamp-2">{badge.description}</p>
                  <div className="mt-2 text-xs text-muted-foreground">{badge.total_awarded || 0} marta berilgan</div>
                  <Button size="sm" variant="ghost"
