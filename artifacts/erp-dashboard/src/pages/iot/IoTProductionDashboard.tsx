...
export function IoTProductionDashboard(props: IoTProductionDashboardProps) {
  ...
  return (
    <div className="min-h-screen bg-background font-inter" data-testid="iot-tablet-production">
      {energySaving && (
        <EnergySavingOverlay currentTime={currentTime} onDismiss={() => setEnergySaving(false)} />
      )}
      ...
      <Dialog open={showSOSDialog} onOpenChange={setShowSOSDialog}>
        <DialogContent className="max-w-sm bg-card border-border shadow-xl p-6">
          <DialogHeader><DialogTitle className="text-[var(--ep-red)] flex items-center gap-2"><Phone className="h-5 w-5" />{t("pdSosTitle")}</DialogTitle></DialogHeader>
          <div className="space-y-3">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
              {([
                { value: "equipment" as const, label: t("pdSosEquipment") },
                { value: "health" as const, label: t("pdSosHealth") },
                { value: "material" as const, label: t("pdSosMaterial") },
                { value: "other" as const, label: t("pdSosOther") },
              ]).map(opt => (
                <Button
                  key={opt.value}
            ...
            <Textarea
              placeholder={t("pdSosPlaceholder")}
              value={sosMessage}
            ...
            <Button variant="destructive" className="w-full h-14 text-lg font-bold bg-[var(--ep-red)] text-white rounded-xl gap-2" onClick={handleSOSSend} data-testid="button-sos-send">
              <Phone className="h-6 w-6" />{t("pdSosSend")}
            </Button>
          </div>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={showHandoverDialog} onOpenChange={setShowHandoverDialog}>
        <DialogContent className="max-w-md bg-card border-border shadow-xl p-6">
          <DialogHeader><DialogTitle className="text-2xl font-bold text-foreground">{t("pdHandoverTitle")}</DialogTitle></DialogHeader>
          <div className="space-y-4">
            {([
              { key: "machineStatus", label: t("pdMachineStatusLabel") },
              { key: "pendingTasks", label: t("pdHandoverPendingTasks") },
              { key: "qualityIssues", label: t("pdHandoverQualityIssues") },
              { key: "safetyNotes", label: t("pdHandoverSafetyNotes") },
              { key: "materialStatus", label: t("pdHandoverMaterialStatus") },
            ]).map(({ key, label }) => (
              <div key={key} className="space-y-1.5">
                <Label className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">{label}</Label>
                <Textarea value={handoverNotes[key as keyof typeof handoverNotes]} onChange={e => setHandoverNotes({ ...handoverNotes, [key]: e.target.value })} placeholder={`${label}...`} className="bg-background border-border text-foreground text-base h-20" data-testid={`handover-${key}`} />
              </div>
            ))}
            <div className="space-y-1.5 pt-2">
              <Label className="text-xs font-bold uppercase tracking-wider text-[var(--ep-red)]">{t("pdSignatureLabel")}</Label>
              <Input value={handoverSignature} onChange={e => setHandoverSignature(e.target.value)} placeholder={t("pdSignaturePlaceholder")} className="bg-background border-border text-foreground h-12 text-lg font-medium" data-testid="input-handover-signature" />
              <p className="text-xs text-muted-foreground font-medium italic">{t("pdSignatureNote")}</p>
            </div>
      ...
      <Dialog open={showQcFormDialog} onOpenChange={setShowQcFormDialog}>
        <DialogContent className="max-w-sm bg-card border-border shadow-xl p-6">
          <DialogHeader><DialogTitle className="text-2xl font-bold text-foreground">{t("pdQcTitle")}</DialogTitle></DialogHeader>
          <div className="space-y-4 py-2">
            <p className="text-sm font-semibold text-muted-foreground bg-muted/60 px-3 py-1.5 rounded-lg border border-border/30">{t("pdCurrentQty", { qty: activeSession?.actualQuantity ?? 0 })}</p>
            <div className="space-y-1.5">
              <Label className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">{t("pdSampleSizeLabel")}</Label>
            ...
            <div className="space-y-1.5">
              <Label className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">{t("pdDefectCountLabel")}</Label>
            ...
            <div className="space-y-1.5">
              ...
              <Textarea value={qcNotes} onChange={e => setQcNotes(e.target.value)} placeholder={t("pdNotesPlaceholderShort")} rows={3} className="bg-background border-border text-foreground text-base" data-testid="input-qc-notes" />
            </div>
          </div>
          <div className="flex gap-3">
            <Button variant="outline" className="flex-1 h-14 text-lg font-bold border-border text-muted-foreground hover:bg-muted" onClick={() => setShowQcFormDialog(false)} data-testid="button-qc-cancel">{t("pdCancel")}</Button>
            <Button className="flex-1 h-14 text-lg font-bold bg-primary text-white" onClick={() => submitInlineQC.mutate()} disabled={submitInlineQC.isPending} data-testid="button-qc-submit">
              {submitInlineQC.isPending ? <EPLoader size={20} className="mr-2" /> : null}{t("pdSave")}
            </Button>
          </div>
        </DialogContent>
      </Dialog>
      ...
      <div className="p-4 space-y-4">
        <OrderCard
          activeSession={activeSession} selectedOrder={selectedOrder}
        ...
        />
        <MetricsCards
          elapsedTime={elapsedTime} activeSession={activeSession}
          estimatedTimeSeconds={estimatedTimeSeconds} formatTime={formatTime} formatEstimatedTime={formatEstimatedTime}
        />
        <MachineStatusPanel
          activeSession={activeSession} sessionEquipmentName={sessionEquipmentName}
          effectiveStatus={effectiveStatus} isAutoStopped={isAutoStopped} secondsSinceLastSignal={secondsSinceLastSignal}
        />
        {qcReminderVisible && (
          <QcReminderBanner activeSession={activeSession} onOpen={() => setShowQcFormDialog(true)} />
        )}
        <ProgressSection progressPercent={progressPercent} activeSession={activeSession} />
        <ActionButtons
      ...
      <BottomActionsBar
        activeSession={activeSession} stopSession={stopSession}
