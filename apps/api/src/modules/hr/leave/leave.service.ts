...
export class LeaveService {
  ...
  constructor(@Inject(HR_LEAVE_SVC_REPO) private readonly hrLeaveSvcRepo: IHrLeaveSvcRepository) {}
  ...
  async findOne(id: number) {
    ...
    if (!result.ok) throw new InternalServerErrorException(result.error);
    if (!result.data) throw new NotFoundException(`Sorov #${id} topilmadi`);
    return result.data;
  }
  ...
  async create(dto: Record<string, unknown>){
    ...
    if (!userResult.data) throw new NotFoundException(`Foydalanuvchi #${dto.userId} topilmadi`);
  ...
  async approve(id: number){
    ...
    if (leave.status !== 'pending') throw new BadRequestException('Faqat pending sorovni tasdiqlash mumkin');
