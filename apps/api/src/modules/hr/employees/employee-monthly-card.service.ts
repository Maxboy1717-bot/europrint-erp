...
export class EmployeeMonthlyCardService {
  ...
  async buildCard(employeeId: number, year: number, month: number): Promise<Result<MonthlyCard>> {
    const rows = await db.execute(sql`
      ...
      LIMIT 1
    `).catch((e: unknown) => {
      this.logger.error('buildCard DB error: %s', String(e));
      return null;
    });
    if (!rows) return Err('DB_ERROR');
    ...
    if (!r) return Err('Employee not found');
    ...
    await db.execute(sql`
      ...
      DO UPDATE SET
        ...
        generated_at = NOW()
    `).catch((e: unknown) => {
      this.logger.warn('buildCard persist error: %s', String(e));
    });
