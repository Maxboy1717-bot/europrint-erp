...
export class ExportRepository {
  async queryEmployeesCsv(): Promise<Result<string>> {
    ...
    return safeCall(async () => {
      const r = await exec(sql`SELECT e.employee_code, u.full_name, u.email, d.name AS department, p.title AS position, e.hire_date::text, e.is_active, e.status FROM employees e LEFT JOIN users u ON u.id = e.user_id LEFT JOIN departments d ON d.id = e.department_id LEFT JOIN positions p ON p.id = e.position_id ORDER BY d.name, u.full_name`);
  ...
  async queryAttendanceCsv(): Promise<Result<string>> {
    ...
    return safeCall(async () => {
      const r = await exec(sql`SELECT u.full_name, d.name AS department, DATE(a.check_in)::text AS date, a.check_in::text, a.check_out::text, a.status, a.work_hours::text FROM attendance a LEFT JOIN employees emp ON emp.id = a.employee_id LEFT JOIN users u ON u.id = emp.user_id LEFT JOIN departments d ON d.id = emp.department_id WHERE a.check_in >= NOW() - INTERVAL '30 days' ORDER BY date DESC, u.full_name`);
  ...
  async queryHrStats(): Promise<Result<HrStatsRow>> {
    ...
    return safeCall(async () => {
      ...
      const emp = empR[0] ?? {};
      const att = attR[0] ?? {};
      return { total: String(emp.total ?? '0'), active: String(emp.active ?? '0'), present: String(att.present ?? '0'), depts: (depR ?? []).map(r => ({ name: String(r.name ?? ''), cnt: String(r.cnt ?? '0') })) };
    }, 'DB_ERROR');
  }
