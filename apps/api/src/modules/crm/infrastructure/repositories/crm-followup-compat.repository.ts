...
import type { ICrmFollowupCompatRepo } from '../../domain/repositories/i-crm-followup-compat.repo';
...
@Injectable()
export class CrmFollowupCompatRepository implements ICrmFollowupCompatRepo {
  ...
  async create(body: Record<string, unknown>): Promise<Result<Row>> {
    try {
      const rows = await exec(sql`
        INSERT INTO crm_activities (type, subject, notes, due_date, entity_type, entity_id, assigned_to, status, created_at)
        VALUES (
    ...
    try {
      const rows = await exec(sql`
        ...
        SET
          ...
          notes      = COALESCE(${(body.note ?? body.notes ?? null) as string | null}, notes),
