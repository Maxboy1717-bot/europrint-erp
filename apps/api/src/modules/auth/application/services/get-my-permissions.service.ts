/**
 * GetMyPermissions service — foydalanuvchining ruxsat to'plamini yig'adi.
 *
 ...
 *
 * Service (formerly get-my-permissions.handler). Never implemented `IQueryHandler<T>`
 * and never registered with the CQRS bus — the controller injects it directly.
 * Renamed handler→service so the location matches its real role (audit P0-7).
 */
...
export class GetMyPermissionsService {
  private readonly logger = new Logger(GetMyPermissionsService.name);
