...
export class PosMiniAppRepository {
  ...
  async searchMaterials(search: string, warehouseId?: string): Promise<Result<Row[]>> {
    return safeCall(async () => {
      return warehouseId
        ? exec(sql`SELECT mc.id, mc.xom_ashyo AS name, mc.xom_ashyo_ru AS "nameRu", mc.barcode, mc.unit_of_measure AS unit, mc.is_consumable AS "isConsumable", mc.min_interval_days AS "minIntervalDays", mc.max_qty_per_issue AS "maxQtyPerIssue", COALESCE(cs.quantity_on_hand, 0) AS stock FROM material_cards mc LEFT JOIN current_stock cs ON cs.material_card_id = mc.id AND cs.warehouse_id = ${warehouseId} WHERE mc.is_active = TRUE AND (mc.xom_ashyo ILIKE ${search} OR mc.xom_ashyo_ru ILIKE ${search} OR mc.barcode ILIKE ${search}) ORDER BY mc.xom_ashyo LIMIT 20`)
        : exec(sql`SELECT mc.id, mc.xom_ashyo AS name, mc.xom_ashyo_ru AS "nameRu", mc.barcode, mc.unit_of_measure AS unit, mc.is_consumable AS "isConsumable", mc.min_interval_days AS "minIntervalDays", mc.max_qty_per_issue AS "maxQtyPerIssue", COALESCE(cs.quantity_on_hand, 0) AS stock FROM material_cards mc LEFT JOIN current_stock cs ON cs.material_card_id = mc.id WHERE mc.is_active = TRUE AND (mc.xom_ashyo ILIKE ${search} OR mc.xom_ashyo_ru ILIKE ${search} OR mc.barcode ILIKE ${search}) ORDER BY mc.xom_ashyo LIMIT 20`);
      }, 'DB_ERROR');
  }
  ...
  async getHistory(userId: number, lmt: number, ofs: number): Promise<Result<{ items: Row[]; total: number }>> {
    return safeCall(async () => {
      const [items, countRows] = await Promise.all([
        exec(sql`SELECT pmr.id, pmr.request_number AS "requestNumber", pmr.status, pmr.priority, pmr.notes, pmr.created_at AS "createdAt", pmr.approved_at AS "approvedAt", pmr.department_code AS "departmentCode", (SELECT json_agg(json_build_object('materialId', prl.material_card_id, 'name', mc.xom_ashyo, 'nameRu', mc.xom_ashyo_ru, 'requestedQty', prl.requested_qty, 'issuedQty', prl.issued_qty, 'unit', mc.unit_of_measure)) FROM pos_material_request_lines prl JOIN material_cards mc ON mc.id = prl.material_card_id WHERE prl.request_id = pmr.id) AS lines FROM pos_material_requests pmr WHERE pmr.requested_by = ${userId} ORDER BY pmr.created_at DESC LIMIT ${lmt} OFFSET ${ofs}`),
