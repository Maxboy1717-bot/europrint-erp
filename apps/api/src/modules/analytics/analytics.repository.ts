...
export class AnalyticsRepository {
  ...
  async findCourseProgress(): Promise<Result<CourseProgressRow[]>> {
    ...
    return safeCall(async () => {
      ...
      return (rows ?? []).map(row => { const en=ni(row.enrolled); const co=ni(row.completed); return { courseId: row.id, courseTitle: row.title??'', enrolled: en, completed: co, completionRate: en>0?Math.round((co/en)*100):0 }; });
    }, 'DB_ERROR');
  }
  ...
  async findUserActivity(): Promise<Result<UserActivityRow[]>> {
    ...
    return safeCall(async () => {
      ...
      return (rows ?? []).map(row => ({ date: row.date, activeUsers: ni(row.active_users) }));
    }, 'DB_ERROR');
  }
  ...
  async findTestResults(): Promise<Result<TestResultRow[]>> {
    ...
    return safeCall(async () => {
      ...
      return (rows ?? []).map(row => { const tot=ni(row.total); const pas=ni(row.passed); return { testId: row.id, testTitle: row.title??'', totalAttempts: tot, passRate: tot>0?Math.round((pas/tot)*100):0, averageScore: np(row.avg_score) }; });
    }, 'DB_ERROR');
  }
  ...
  async findByDepartment(): Promise<Result<DeptRow[]>> {
    ...
    return safeCall(async () => {
      ...
      return (rows ?? []).map(row => ({ departmentId: row.id, departmentName: row.name??'', userCount: ni(row.user_count), completionRate: np(row.completion_rate), averageScore: np(row.avg_score) }));
    }, 'DB_ERROR');
  }
  ...
  async findByPosition(): Promise<Result<PosRow[]>> {
    ...
    return safeCall(async () => {
      ...
      return (rows ?? []).map(row => ({ positionId: row.id, positionName: row.title??'', userCount: ni(row.user_count), completionRate: 0, averageScore: np(row.avg_score) }));
    }, 'DB_ERROR');
  }
  ...
  async findLeaderboardDepartments(): Promise<Result<LeaderDeptRow[]>> {
    ...
    return safeCall(async () => {
      ...
      return (rows ?? []).map(row => { const tot=ni(row.total); const comp=ni(row.completed); return { departmentId: row.id, departmentName: row.name??'', userCount: ni(row.user_count), completedAssignments: comp, totalAssignments: tot, completionRate: tot>0?Math.round((comp/tot)*100):0, averageScore: np(row.avg_score) }; });
    }, 'DB_ERROR');
  }
