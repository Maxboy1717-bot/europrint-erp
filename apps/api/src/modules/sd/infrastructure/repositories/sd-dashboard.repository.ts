...
export class SdDashboardRepository implements ISdDashboardRepo {
  ...
  try {
      return mid
        ? exec(sql`SELECT e.id AS manager_id, CONCAT(e.first_name, ' ', e.last_name) AS manager_name, COALESCE(SUM(o.total_value), 0)::numeric(15,2) AS achieved, COALESCE(MAX(q.quota_amount), 0)::numeric(15,2) AS target, COUNT(o.id)::int AS order_count FROM employees e LEFT JOIN crm_leads l ON l.manager_id = e.id LEFT JOIN sales_orders o ON o.customer_id = l.customer_id AND DATE(o.created_at) >= DATE_TRUNC('month', NOW()) LEFT JOIN sd_manager_quotas q ON q.manager_id = e.id AND q.year = EXTRACT(YEAR FROM NOW())::int AND q.month = EXTRACT(MONTH FROM NOW())::int WHERE e.id = ${mid} GROUP BY e.id, e.first_name, e.last_name ORDER BY achieved DESC`)
        : exec(sql`SELECT e.id AS manager_id, CONCAT(e.first_name, ' ', e.last_name) AS manager_name, COALESCE(SUM(o.total_value), 0)::numeric(15,2) AS achieved, COALESCE(MAX(q.quota_amount), 0)::numeric(15,2) AS target, COUNT(o.id)::int AS order_count FROM employees e LEFT JOIN crm_leads l ON l.manager_id = e.id LEFT JOIN sales_orders o ON o.customer_id = l.customer_id AND DATE(o.created_at) >= DATE_TRUNC('month', NOW()) LEFT JOIN sd_manager_quotas q ON q.manager_id = e.id AND q.year = EXTRACT(YEAR FROM NOW())::int AND q.month = EXTRACT(MONTH FROM NOW())::int GROUP BY e.id, e.first_name, e.last_name ORDER BY achieved DESC LIMIT 20`);  } catch (_e) {
    return Ok([]);
  }
