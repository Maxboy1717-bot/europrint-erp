...
export class GetGlEntriesHandler implements IQueryHandler<GetGlEntriesQuery> {
  ...
  async execute(query: GetGlEntriesQuery): Promise<Result<PaginatedResult<Record<string, unknown>>>> {
    ...
    try {
      ...
      return Err(AppErr('INTERNAL', 'Failed to fetch GL entries'));
    }
