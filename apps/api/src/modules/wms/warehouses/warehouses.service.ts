...
export class WarehousesService {
  ...
  constructor(@Inject(WMS_WAREHOUSES_REPO) private readonly wmsWarehousesRepo: IWmsWarehousesRepository) {}
  ...
  async findOne(id: number) {
    ...
    if (!result.ok) throw new InternalServerErrorException(result.error);
    if (!result.data) throw new NotFoundException(`Ombor #${id} topilmadi`);
  ...
  async deactivate(id: number){
    ...
    if (!result.ok) throw new InternalServerErrorException(result.error);
    return { message: "Ombor deaktiv qilindi" };
