...
export async function ensureDbInvariants(): Promise<void> {
  const constraints: Array<{ name: string; table: string; check: string }> = [
    ...
    },
    {
      ...
      check: 'planned_quantity::numeric > 0', // column is planned_quantity per migration 0000
    },
    {
      name: 'chk_production_orders_qty_non_negative',
      table: 'production_orders',
      check: 'confirmed_quantity::numeric >= 0', // column is confirmed_quantity per migration 0000
    },
    // ── Maosh (HR) ───────────────────────────────────────────────
    {
    ...
    // NOTE: a unified `invoices` table does not exist in migration 0000 — only
    // crm_invoices, purchase_invoices, sales_invoices, vendor_invoices. These
    // CHECK definitions are forward-looking and fail silently on tables that
    // don't exist yet (the runtime guards missing-table errors). When/if a
    // unified `invoices` table is introduced, these CHECKs become live.
    {
    ...
    // NOTE: purchase_orders.quantity does not yet exist (migration 0000 only
    // models total_amount). The CHECK is forward-looking and silently no-ops
    // until the column is added; it documents the intended invariant.
    {
  ...
  for (const c of constraints) {
    try {
      ...
      skipped++;
      // Known missing tables (invoices, purchase_orders.quantity) log at debug to avoid noise
...
export async function ensureSchemaAdditions(): Promise<void> {
  const migrations: Array<{ name: string; sql: string }> = [
    ...
    // ── CRM core tables (create if missing, add columns if old Bitrix schema exists) ──
    {
    ...
    {
      name: 'crm_activities add lead_id col if missing',
      sql: `ALTER TABLE crm_activities ADD COLUMN IF NOT EXISTS lead_id INTEGER`,
    },
    {
      name: 'crm_activities add deal_id col if missing',
      sql: `ALTER TABLE crm_activities ADD COLUMN IF NOT EXISTS deal_id INTEGER`,
    },
    {
      name: 'crm_activities add notes col if missing',
      sql: `ALTER TABLE crm_activities ADD COLUMN IF NOT EXISTS notes TEXT`,
    },
    {
      name: 'crm_activities add subject col if missing',
      sql: `ALTER TABLE crm_activities ADD COLUMN IF NOT EXISTS subject TEXT`,
    },
    {
      name: 'crm_activities add due_date col if missing',
      sql: `ALTER TABLE crm_activities ADD COLUMN IF NOT EXISTS due_date TIMESTAMP`,
    },
    {
      name: 'crm_activities add status col if missing',
      sql: `ALTER TABLE crm_activities ADD COLUMN IF NOT EXISTS status TEXT DEFAULT 'scheduled'`,
    },
    {
      name: 'crm_activities add outcome col if missing',
      sql: `ALTER TABLE crm_activities ADD COLUMN IF NOT EXISTS outcome TEXT`,
    },
    {
      name: 'crm_activities add completed_at col if missing',
      sql: `ALTER TABLE crm_activities ADD COLUMN IF NOT EXISTS completed_at TIMESTAMP`,
    },
    {
      name: 'crm_activities add assigned_to col if missing',
      sql: `ALTER TABLE crm_activities ADD COLUMN IF NOT EXISTS assigned_to INTEGER`,
    },
    {
      name: 'crm_activities add created_at col if missing',
      sql: `ALTER TABLE crm_activities ADD COLUMN IF NOT EXISTS created_at TIMESTAMP DEFAULT now()`,
    },
    {
      name: 'crm_activities add updated_at col if missing',
      sql: `ALTER TABLE crm_activities ADD COLUMN IF NOT EXISTS updated_at TIMESTAMP DEFAULT now()`,
    },
    {
      name: 'crm_activities add entity_type col if missing',
      sql: `ALTER TABLE crm_activities ADD COLUMN IF NOT EXISTS entity_type TEXT`,
    },
    {
      name: 'crm_activities add entity_id col if missing',
      sql: `ALTER TABLE crm_activities ADD COLUMN IF NOT EXISTS entity_id INTEGER`,
    },
    {
      name: 'crm_activities add type col if missing',
      sql: `ALTER TABLE crm_activities ADD COLUMN IF NOT EXISTS type TEXT`,
    },
    {
      name: 'crm_activities add scheduled_at col if missing',
      sql: `ALTER TABLE crm_activities ADD COLUMN IF NOT EXISTS scheduled_at TIMESTAMP`,
    },
    {
      name: 'crm_activities add assignee_id col if missing',
      sql: `ALTER TABLE crm_activities ADD COLUMN IF NOT EXISTS assignee_id INTEGER`,
    },
    {
      name: 'crm_activities add created_by col if missing',
      sql: `ALTER TABLE crm_activities ADD COLUMN IF NOT EXISTS created_by INTEGER`,
    },
    {
      name: 'crm_activities drop NOT NULL on owner_type_id',
      sql: `ALTER TABLE crm_activities ALTER COLUMN owner_type_id DROP NOT NULL`,
    },
    {
      name: 'crm_activities drop NOT NULL on owner_id',
      sql: `ALTER TABLE crm_activities ALTER COLUMN owner_id DROP NOT NULL`,
    },
    {
      name: 'crm_activities drop NOT NULL on type_id',
      sql: `ALTER TABLE crm_activities ALTER COLUMN type_id DROP NOT NULL`,
    },
    {
      name: 'crm_activities drop NOT NULL on responsible_id',
      sql: `ALTER TABLE crm_activities ALTER COLUMN responsible_id DROP NOT NULL`,
    },
    {
      name: 'crm_comments drop NOT NULL on entity_type',
      sql: `ALTER TABLE crm_comments ALTER COLUMN entity_type DROP NOT NULL`,
    },
    {
      name: 'crm_comments drop NOT NULL on entity_id',
      sql: `ALTER TABLE crm_comments ALTER COLUMN entity_id DROP NOT NULL`,
    },
    {
      name: 'crm_comments drop NOT NULL on content',
      sql: `ALTER TABLE crm_comments ALTER COLUMN content DROP NOT NULL`,
    },
    {
      name: 'crm_comments drop NOT NULL on author_id',
      sql: `ALTER TABLE crm_comments ALTER COLUMN author_id DROP NOT NULL`,
    },
    {
    ...
    {
      name: 'crm_comments add lead_id col if missing',
      sql: `ALTER TABLE crm_comments ADD COLUMN IF NOT EXISTS lead_id INTEGER`,
    },
    {
      name: 'crm_comments add deal_id col if missing',
      sql: `ALTER TABLE crm_comments ADD COLUMN IF NOT EXISTS deal_id INTEGER`,
    },
    {
      name: 'crm_comments add text col if missing',
      sql: `ALTER TABLE crm_comments ADD COLUMN IF NOT EXISTS text TEXT`,
    },
    {
      name: 'crm_comments add author_id col if missing',
      sql: `ALTER TABLE crm_comments ADD COLUMN IF NOT EXISTS author_id INTEGER`,
    },
    {
    ...
    {
      name: 'crm_tasks add lead_id col if missing',
      sql: `ALTER TABLE crm_tasks ADD COLUMN IF NOT EXISTS lead_id INTEGER`,
    },
    {
      name: 'crm_tasks add deal_id col if missing',
      sql: `ALTER TABLE crm_tasks ADD COLUMN IF NOT EXISTS deal_id INTEGER`,
    },
    {
      name: 'crm_tasks add priority col if missing',
      sql: `ALTER TABLE crm_tasks ADD COLUMN IF NOT EXISTS priority TEXT DEFAULT 'medium'`,
    },
    {
      name: 'crm_tasks add status col if missing',
      sql: `ALTER TABLE crm_tasks ADD COLUMN IF NOT EXISTS status TEXT DEFAULT 'pending'`,
    },
    {
      name: 'crm_tasks add due_date col if missing',
      sql: `ALTER TABLE crm_tasks ADD COLUMN IF NOT EXISTS due_date TIMESTAMP`,
    },
    {
      name: 'crm_tasks add assigned_to col if missing',
      sql: `ALTER TABLE crm_tasks ADD COLUMN IF NOT EXISTS assigned_to INTEGER`,
    },
    // ── crm_leads: add missing compat columns for AI scoring ───────────────
    {
      name: 'crm_leads add contact_name col if missing',
      sql: `ALTER TABLE IF EXISTS crm_leads ADD COLUMN IF NOT EXISTS contact_name VARCHAR(200)`,
    },
    {
      name: 'crm_leads add contact_email col if missing',
      sql: `ALTER TABLE IF EXISTS crm_leads ADD COLUMN IF NOT EXISTS contact_email VARCHAR(200)`,
    },
    {
      name: 'crm_leads add contact_phone col if missing',
      sql: `ALTER TABLE IF EXISTS crm_leads ADD COLUMN IF NOT EXISTS contact_phone VARCHAR(50)`,
    },
    // ── Warehouses cleanup: deactivate old WH-* warehouses ─────────────────
    // POS Monitor va ERP Ombor moduli bitta omborlar to'plamini ko'rsatsin.
    // Faqat 9 ta rasmiy ombor faol bo'ladi (RM-MAIN, RM-ROLLS, FG-MAIN,
    // WIP-MAIN, SCRAP-MAIN, QC-HOLD, TOOL-MAIN, MRO-MAIN, MRO-STORE).
    {
    ...
    // ── Warehouses seed: 9 ta standart ombor (idempotent) ──────────────────
    {
    ...
    // ── pos_movement_types seed: 7 ta harakat turi ─────────────────────────
    // Spec ga muvofiq: EXTERNAL_IN, EXTERNAL_OUT, INTERNAL_ISSUE, INTERNAL_RETURN,
    // INTERNAL_TRANSFER, DAMAGE, INVENTORY_ADJUST
    //
    // Jadvalda faqat 5 ta ustun mavjud (id, code, name, direction, is_active, created_at).
    // `name_ru` va `requires_document` ustunlari jadvalda yo'q.
    // `id` SERIAL emas, shu sababli COALESCE(MAX(id),0)+1 bilan qo'lda beriladi.
    {
