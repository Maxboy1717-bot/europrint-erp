-- Migration: Financial Reports snapshot tables
-- Deterministic text PKs (e.g. 'kassa-2026-05-08') so cron snapshots are idempotent.

-- 1. Kassa (Cash Flow) daily snapshots
CREATE TABLE IF NOT EXISTS rpt_kassa_transactions (
  id                TEXT          PRIMARY KEY,
  report_date       DATE          NOT NULL,
  total_inflow      NUMERIC(18,4) NOT NULL DEFAULT 0 CHECK (total_inflow >= 0),
  total_outflow     NUMERIC(18,4) NOT NULL DEFAULT 0 CHECK (total_outflow >= 0),
...
CREATE TABLE IF NOT EXISTS rpt_ombor_qoldiq (
  ...
  overstock_flag    TEXT          DEFAULT 'normal' CHECK (overstock_flag IN ('normal','warning','critical')),
  details           JSONB,
  created_at        TIMESTAMPTZ   NOT NULL DEFAULT NOW()
);
...
CREATE TABLE IF NOT EXISTS rpt_debitorlar (
  ...
  customer_id       INTEGER,
  customer_name     TEXT,
  total_receivable  NUMERIC(18,4) NOT NULL DEFAULT 0 CHECK (total_receivable >= 0),
...
CREATE TABLE IF NOT EXISTS rpt_kreditorlar (
  ...
  vendor_id         INTEGER,
  vendor_name       TEXT,
  total_payable     NUMERIC(18,4) NOT NULL DEFAULT 0 CHECK (total_payable >= 0),
  current           NUMERIC(18,4) NOT NULL DEFAULT 0,
  overdue_30        NUMERIC(18,4) NOT NULL DEFAULT 0,
  overdue_60        NUMERIC(18,4) NOT NULL DEFAULT 0,
  overdue_90_plus   NUMERIC(18,4) NOT NULL DEFAULT 0,
...
CREATE TABLE IF NOT EXISTS rpt_ishlab_chiqarish (
  ...
  efficiency_pct   NUMERIC(18,4) NOT NULL DEFAULT 0 CHECK (efficiency_pct >= 0 AND efficiency_pct <= 100),
  scrap_rate_pct   NUMERIC(18,4) NOT NULL DEFAULT 0 CHECK (scrap_rate_pct >= 0 AND scrap_rate_pct <= 100),
