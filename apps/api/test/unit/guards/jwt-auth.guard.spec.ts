...
describe('JwtAuthGuard', () => {
  ...
  it('throws UnauthorizedException when token is expired', async () => {
    ...
    const guard = new JwtAuthGuard(makeReflector(false), makeJwt(expired));
  ...
  it('returns true and attaches user when token is valid', async () => {
    ...
    const guard = new JwtAuthGuard(makeReflector(false), makeJwt(decoded));
  ...
  it('throws UnauthorizedException when payload has no user identifier', async () => {
    const guard = new JwtAuthGuard(makeReflector(false), makeJwt({ name: 'orphan' }));
