...
import { pgTable, text, varchar, integer, boolean, timestamp, jsonb, serial, numeric, pgSequence } from "drizzle-orm/pg-core";
...
export const designOrders = pgTable("design_orders", {
  ...
  assignedDesignerId: integer("assigned_designer_id").references(() => users.id),
...
export const designOrderMessages = pgTable("designOrderMessages", {
  ...
  senderId: varchar("senderId").notNull().references(() => users.id),
...
export const designOrderNotifications = pgTable("designOrderNotifications", {
  id: serial("id").primaryKey(),
  orderId: varchar("orderId").notNull().references(() => designOrders.id, { onDelete: "cascade" }),
  recipientId: varchar("recipientId").notNull().references(() => users.id),
