...
export const glLines = pgTable("gl_lines", {
  ...
  // NOTE: cost_centers/profit_centers pgTables removed (orphan, lib/db-only, no @workspace/db
  // or @europrint/schemas consumer — see chore(schema) cleanup 2026-07-02). Columns kept
  // plain (no cross-table FK type) to preserve the DB column without a dangling reference.
  costCenterId: varchar("cost_center_id"),
  profitCenterId: varchar("profit_center_id"),
...
// ORFAN CLEANUP (2026-07-02): payrollRows pgTable removed — dead lib/db duplicate,
// Q-29 verified: never imported via @workspace/db/@europrint/schemas; canonical
// live payroll_rows table is apps/api/src/shared/db/schema-compat-2.ts.
