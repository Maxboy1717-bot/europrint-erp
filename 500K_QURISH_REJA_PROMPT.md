...
`SD→PP` ✅ (avto avans→PO, `59341adf`, real SO.69→PO.66 proof) → `PP→MES` ✅ (PpReleasedMesListener ALLAQACHON ishlaydi: release PO.67→session.60 bog'landi, idempotent, proof OK; uuid two-worlds = ishlatilmaydigan o'lik schema-manufacturing.ts binding, operatsion yo'l integer — kod o'zgartirish kerak emas) → **`MES→QC`** → `QC→WMS` → `WMS→FIN(invoice)`. Zanjir to'liq bo'lgach → keyingi feature (gofra-hisob, AI-karta — egasi-DATA bilan).
