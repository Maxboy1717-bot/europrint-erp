...
`SD→PP` ✅ (avto avans→PO, `59341adf`, real SO.69→PO.66 proof) → `PP→MES` ✅ (PpReleasedMesListener ALLAQACHON ishlaydi: release PO.67→session.60 bog'landi, idempotent, proof OK; uuid two-worlds = ishlatilmaydigan o'lik schema-manufacturing.ts binding, operatsion yo'l integer — kod o'zgartirish kerak emas) → `MES→QC` ✅ (mes-completed.listener ALLAQACHON ishlaydi: session complete→qc_inspection.41 order_id-bog'landi, work-list'da ko'rindi, proof OK; "no-op stub"+"production_order_id two-worlds" = scope yashil-yolg'on 5,6 — jonli order_id integer, kod-o'zgarmas) → `QC→WMS` ✅ (QcPassedListener→ReceiveFg ALLAQACHON ishlaydi: REAL PERSISTENT data — qc_inspection.34 passed→warehouse_stock.75/76 FG qty 4900/2450, 2026-06-20; integer keys, two-worlds yo'q; minor: warehouse_stock'da production_order_id FK yo'q [batch_number string] — kelajak enhancement) → `WMS→FIN(invoice)` ⛔ GATED (JONLI DB-proof: 3 invoice-jadval — `invoices`.sales_order_id=uuid/7-qator · `sales_invoices`.sales_order_id=text/0 · `fi_invoices` — hech biri integer `sales_orders.id`ga ulanmaydi; auto-link yo'q [manual POST]; wiring `create-invoice.handler`=LOCKED. Bloker: kanonik invoice-jadval qarori + LOCKED handler).

> 🏁 **GOLDEN-THREAD YAKUN (2026-06-23):** ERP YADRO OQIMI ✅ — `SD→PP→MES→QC→WMS` 4/5 bo'g'in UCHMA-UCH ISBOTLANGAN (real/persistent data): buyurtma tasdiq+avans → ishlab-chiqarish buyurtmasi (59341adf real fix) → MES sessiya → QC tekshiruv → tayyor-mahsulot ombori (warehouse_stock.75/76 real qty 4900/2450). Faqat oxirgi `WMS→FIN(invoice)` bo'g'ini gated (invoice-fragmentatsiya + LOCKED handler). Ochish uchun egasi/arxitektura: **kanonik invoice-jadval** (invoices uuid yoki sales_invoices→integer konvergatsiya) + create-invoice.handler LOCKED'ni ochish.
> ⚠️ Q-29 SABOQ: PP→MES scope (wlljpwk5l) "buzuq INSERT + uuid╳int two-worlds, UUID konvertatsiya kerak" dedi — **to'liq yashil-yolg'on** (4-marta). Jonli DB: production_sessions.production_order_id=integer, hamma listener-ustun mavjud. O'z DB-proof'im buzuq UUID-migratsiyadan saqladi. Har scope-da'vo jonli tekshiriladi.
